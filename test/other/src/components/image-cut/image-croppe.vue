  ...
  <view class="vue-cropper" ref="cropper" :style="{ top : `${containerTop}px` }" v-show="show">
    <view class="cropper-box">
      <view
        ...
        :style="{'width': imageWidth + 'px','height': imageHeight + 'px','transform': 'scale(' + scale + ',' + scale + ') ' + 'translate3d('+ x / scale + 'px,' + y / scale + 'px,' + '0)' + 'rotateZ('+ rotate * 90 +'deg)'}"
      >
        <image :src="src" alt="cropper-img" ref="cropperImg" mode="scaleToFill" class="uni-image" />
      </view>
    </view>
    ...
    <cover-view class="btn-wrapper" :class="isIpx ? 'isipx-view' : ''">
      <cover-view @tap="cancel">{{$t('cancel')}}</cover-view>
      <cover-view @tap="confirm">{{$t('Confirm')}}</cover-view>
    </cover-view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    init() {
      this.rotate = 0;
      this.scale = 1;
      this.cropW = parseInt(this.cropWidth) * this.pixelRatio;
      this.cropH = parseInt(this.cropHeight) * this.pixelRatio;
      this.src = this.imageSrc;
      // this.cropW = this.cropWidth
      // this.cropH = this.cropHeight
      uni.showLoading({
        title: this.$t('imageLoading')
      });
      this.loadImage(this.src).then((e) => {
        ...
        uni.showModal({
          title: this.$t('title'),
          content: this.$t('imageLoadingFailed')
        });
      });
    },
    loadImage(src) {
    ...
    confirm(event) {
      ...
      ctx.fillRect(0, 0, _this.cropW, _this.cropH);
      // ctx.fillRect(0, 0, _this.imageWidt, _this.imageHeight);
      ctx.save();
      ...
      ctx.draw(false, () => {
        uni.canvasToTempFilePath({
          ...
          success: (res) => {
            uni.hideLoading();
            console.log(res);
            event.tempFilePath = res.tempFilePath;
