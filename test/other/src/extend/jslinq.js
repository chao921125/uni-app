(function(root, factory) {
  ...
  function jslinq(array) {
    // Check arguments
    if (!array) { throw new Error('Provided data is invalid'); }
    if (!(array instanceof Array)) { throw new Error('Provided data is not a valid array'); }
  ...
  function where(expression) {
    ...
    for (var i = 0; i < this.items.length; i++) {
      ...
      if (doesMatch) { outData.push(this.items[i]); }
    }
  ...
  function join(otherData) {
    ...
    if (!otherData) { return new jslinq(this.items); }
  ...
  function distinct() {
    ...
    for (var i = 0; i < this.items.length; i++) {
      ...
      if (!wasFound) { outData.push(this.items[i]); }
    }
  ...
  function singleOrDefault(expression) {
    ...
    if (outData.length === 0) { return null; }

    // If contains just one element, returns element
    if (outData.length == 1) { return outData[0]; }
  ...
  function skip(value) {
    // Check arguments
    if (value < 0) { throw new Error('Value must be greater or equals zero'); }
  ...
  function intersect(otherData, compareExpression) {
    ...
    if (!otherData) { return new jslinq([]); }
  ...
  function remove(elementToRemove) {
    ...
    if (!elementToRemove) { return new jslinq(this.items); }
    ...
    for (var n = 0; n < this.items.length; n++) {
      ...
      if (equals(this.items[n], elementToRemove)) { continue; }
  ...
  function sum(expression) {
    ...
    var outData = new jslinq(this.items)
      .select(expression)
      .toList();
    ...
    for (var i = 0; i < outData.length; i++) {
      ...
      if (typeof currentValue !== 'number') { throw Error("Value '" + currentValue + "' is not a number"); }
