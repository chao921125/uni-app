<template>
	<view class="body-contains">
		<uni-forms ref="refForm" :modelValue="formData" :rules="rules" validate-trigger="bind" label-width="90" :border="true">
			<uni-forms-item label="用户名" required name="uname">
				<uni-easyinput v-model="formData.uname" maxlength="50" placeholder="请输入用户名" />
			</uni-forms-item>
			...
			<uni-forms-item label="邀请码" required name="inviteCode">
				<uni-easyinput v-model="formData.inviteCode" maxlength="10" placeholder="请输入邀请码" />
			</uni-forms-item>
			<uni-forms-item label="密码" required name="upassword">
				<uni-easyinput type="password" v-model="formData.upassword" maxlength="50" placeholder="请输入密码" />
			</uni-forms-item>
			<uni-forms-item label="确认新密码" required name="rePassword">
				<uni-easyinput type="password" v-model="formData.rePassword" maxlength="50" placeholder="请输入确认新密码" />
			</uni-forms-item>
		</uni-forms>
		<button class="btn-error re-margin-top-50" @click="submit()">注册</button>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getCode(value) {
				this.$refs.refForm.validateField(["tel"]).then(() => {
					getPhoneCode({tel: this.formData.tel, codeType: "1"}).then((res) => {
