			...
			<view class="example">
				...
				<uni-forms ref="baseForm" :model="baseFormData" labelWidth="80px">
					...
					<uni-forms-item label="选择技能">
						<uni-data-select v-model="baseFormData.skills" :localdata="skillsRange"> </uni-data-select>
					</uni-forms-item>
				</uni-forms>
			</view>
		</uni-section>
		...
		<uni-section title="动态表单" type="line">
			<view class="example">
				...
				<uni-forms ref="dynamicForm" :rules="dynamicRules" :model="dynamicFormData" labelWidth="80px">
					...
					<uni-forms-item v-for="(item, index) in dynamicFormData.domains" :key="item.id" :label="item.label + ' ' + index" required :rules="item.rules" :name="['domains', index, 'value']">
