<template>
	...
	<view class="re-flex-row-between re-margin-top-50">
		<view class="re-flex-row-center text-click" @click="toShop">
			<text class="re-flex-row-center re-padding-20 text-circle text-line-height">商城</text>
		</view>
		<view class="re-flex-row-center text-click" @click="toBooking">
			<text class="re-flex-row-center re-padding-20 text-circle text-line-height">预约</text>
		</view>
	</view>
			...
			<uni-grid-item v-for="(item, index) in userAll" :index="index" :key="index">
				<view class="re-flex-column-center">
					<view class="re-flex-row-center">
						<cover-image class="avatar-circle" v-if="item.avatar" :src="item.avatar"></cover-image>
...
<script>
	...
	import { subjectList, expertList } from "@/common/api/shop.js";
    ...
    export default {
		data() {
			return {
				...
				userAll: [],
				requestUrl: requestConfig.getBaseUrl()
			}
		},
		onShow() {
			utils.checkPermiss();
