!(function (A) {
	var B = {};
	function t(e) {
		if (B[e]) return B[e].exports;
		var E = (B[e] = { i: e, l: !1, exports: {} });
		return A[e].call(E.exports, E, E.exports, t), (E.l = !0), E.exports;
	}
	(t.m = A),
		(t.c = B),
		(t.d = function (A, B, e) {
			t.o(A, B) || Object.defineProperty(A, B, { enumerable: !0, get: e });
		}),
		(t.r = function (A) {
			"undefined" != typeof Symbol && Symbol.toStringTag && Object.defineProperty(A, Symbol.toStringTag, { value: "Module" }),
				Object.defineProperty(A, "__esModule", { value: !0 });
		}),
		(t.t = function (A, B) {
			if ((1 & B && (A = t(A)), 8 & B)) return A;
			if (4 & B && "object" == typeof A && A && A.__esModule) return A;
			var e = Object.create(null);
			if ((t.r(e), Object.defineProperty(e, "default", { enumerable: !0, value: A }), 2 & B && "string" != typeof A))
				for (var E in A)
					t.d(
						e,
						E,
						function (B) {
							return A[B];
						}.bind(null, E),
					);
			return e;
		}),
		(t.n = function (A) {
			var B =
				A && A.__esModule
					? function () {
							return A.default;
					  }
					: function () {
							return A;
					  };
			return t.d(B, "a", B), B;
		}),
		(t.o = function (A, B) {
			return Object.prototype.hasOwnProperty.call(A, B);
		}),
		(t.p = ""),
		t((t.s = 680));
})({
	0: function (A, B, t) {
		"use strict";
		function e(A, B, t, e, E, c, i, g, n, Q) {
			var o,
				w = "function" == typeof A ? A.options : A;
			if (n) {
				w.components || (w.components = {});
				var a = Object.prototype.hasOwnProperty;
				for (var s in n) a.call(n, s) && !a.call(w.components, s) && (w.components[s] = n[s]);
			}
			if (
				(Q &&
					((Q.beforeCreate || (Q.beforeCreate = [])).unshift(function () {
						this[Q.__module] = this;
					}),
					(w.mixins || (w.mixins = [])).push(Q)),
				B && ((w.render = B), (w.staticRenderFns = t), (w._compiled = !0)),
				e && (w.functional = !0),
				c && (w._scopeId = "data-v-" + c),
				i
					? ((o = function (A) {
							(A = A || (this.$vnode && this.$vnode.ssrContext) || (this.parent && this.parent.$vnode && this.parent.$vnode.ssrContext)) ||
								"undefined" == typeof __VUE_SSR_CONTEXT__ ||
								(A = __VUE_SSR_CONTEXT__),
								E && E.call(this, A),
								A && A._registeredComponents && A._registeredComponents.add(i);
					  }),
					  (w._ssrRegister = o))
					: E &&
					  (o = g
							? function () {
									E.call(this, this.$root.$options.shadowRoot);
							  }
							: E),
				o)
			)
				if (w.functional) {
					w._injectStyles = o;
					var l = w.render;
					w.render = function (A, B) {
						return o.call(B), l(A, B);
					};
				} else {
					var r = w.beforeCreate;
					w.beforeCreate = r ? [].concat(r, o) : [o];
				}
			return { exports: A, options: w };
		}
		t.d(B, "a", function () {
			return e;
		});
	},
	1: function (A, B) {
		A.exports = { "@VERSION": 2 };
	},
	10: function (A, B, t) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e,
			E = (e = t(17)) && e.__esModule ? e : { default: e };
		weex
			.requireModule("dom")
			.addRule("fontFace", {
				fontFamily: "uniicons",
				src: "url('data:font/truetype;charset=utf-8;base64,')",
			});
		var c = {
			name: "UniIcons",
			props: {
				type: { type: String, default: "" },
				color: { type: String, default: "#333333" },
				size: { type: [Number, String], default: 16 },
				customIcons: { type: String, default: "" },
			},
			data: function () {
				return { icons: E.default };
			},
			methods: {
				_onClick: function () {
					this.$emit("click");
				},
			},
		};
		B.default = c;
	},
	11: function (A, B) {
		A.exports = {
			".uni-icons": { "": { fontFamily: ["uniicons", 0, 0, 0], textDecoration: ["none", 0, 0, 0], textAlign: ["center", 0, 0, 0] } },
			"@VERSION": 2,
		};
	},
	12: function (A, B, t) {
		"use strict";
		t.d(B, "b", function () {
			return e;
		}),
			t.d(B, "c", function () {
				return E;
			}),
			t.d(B, "a", function () {});
		var e = function () {
				var A = this,
					B = A.$createElement;
				return (A._self._c || B)(
					"u-text",
					{
						staticClass: ["uni-icons"],
						class: [A.customIcons, A.customIcons ? A.type : ""],
						style: { color: A.color, "font-size": A.size + "px" },
						appendAsTree: !0,
						attrs: { append: "tree" },
						on: { click: A._onClick },
					},
					[A._v(A._s(A.icons[A.type]))],
				);
			},
			E = [];
	},
	13: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(5),
			E = t(2);
		for (var c in E)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return E[A];
					});
				})(c);
		var i = t(0);
		var g = Object(i.a)(E.default, e.b, e.c, !1, null, "168bca6c", "39eb9647", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(t(8).default, this.options.style) : Object.assign(this.options.style, t(8).default);
		}).call(g),
			(B.default = g.exports);
	},
	15: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(11),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	16: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(12),
			E = t(9);
		for (var c in E)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return E[A];
					});
				})(c);
		var i = t(0);
		var g = Object(i.a)(E.default, e.b, e.c, !1, null, "041e7dc0", "67d6d51e", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(t(15).default, this.options.style) : Object.assign(this.options.style, t(15).default);
		}).call(g),
			(B.default = g.exports);
	},
	17: function (A, B, t) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		B.default = {
			pulldown: "\ue588",
			refreshempty: "\ue461",
			back: "\ue471",
			forward: "\ue470",
			more: "\ue507",
			"more-filled": "\ue537",
			scan: "\ue612",
			qq: "\ue264",
			weibo: "\ue260",
			weixin: "\ue261",
			pengyouquan: "\ue262",
			loop: "\ue565",
			refresh: "\ue407",
			"refresh-filled": "\ue437",
			arrowthindown: "\ue585",
			arrowthinleft: "\ue586",
			arrowthinright: "\ue587",
			arrowthinup: "\ue584",
			"undo-filled": "\ue7d6",
			undo: "\ue406",
			redo: "\ue405",
			"redo-filled": "\ue7d9",
			bars: "\ue563",
			chatboxes: "\ue203",
			camera: "\ue301",
			"chatboxes-filled": "\ue233",
			"camera-filled": "\ue7ef",
			"cart-filled": "\ue7f4",
			cart: "\ue7f5",
			"checkbox-filled": "\ue442",
			checkbox: "\ue7fa",
			arrowleft: "\ue582",
			arrowdown: "\ue581",
			arrowright: "\ue583",
			"smallcircle-filled": "\ue801",
			arrowup: "\ue580",
			circle: "\ue411",
			"eye-filled": "\ue568",
			"eye-slash-filled": "\ue822",
			"eye-slash": "\ue823",
			eye: "\ue824",
			"flag-filled": "\ue825",
			flag: "\ue508",
			"gear-filled": "\ue532",
			reload: "\ue462",
			gear: "\ue502",
			"hand-thumbsdown-filled": "\ue83b",
			"hand-thumbsdown": "\ue83c",
			"hand-thumbsup-filled": "\ue83d",
			"heart-filled": "\ue83e",
			"hand-thumbsup": "\ue83f",
			heart: "\ue840",
			home: "\ue500",
			info: "\ue504",
			"home-filled": "\ue530",
			"info-filled": "\ue534",
			"circle-filled": "\ue441",
			"chat-filled": "\ue847",
			chat: "\ue263",
			"mail-open-filled": "\ue84d",
			"email-filled": "\ue231",
			"mail-open": "\ue84e",
			email: "\ue201",
			checkmarkempty: "\ue472",
			list: "\ue562",
			"locked-filled": "\ue856",
			locked: "\ue506",
			"map-filled": "\ue85c",
			"map-pin": "\ue85e",
			"map-pin-ellipse": "\ue864",
			map: "\ue364",
			"minus-filled": "\ue440",
			"mic-filled": "\ue332",
			minus: "\ue410",
			micoff: "\ue360",
			mic: "\ue302",
			clear: "\ue434",
			smallcircle: "\ue868",
			close: "\ue404",
			closeempty: "\ue460",
			paperclip: "\ue567",
			paperplane: "\ue503",
			"paperplane-filled": "\ue86e",
			"person-filled": "\ue131",
			"contact-filled": "\ue130",
			person: "\ue101",
			contact: "\ue100",
			"images-filled": "\ue87a",
			phone: "\ue200",
			images: "\ue87b",
			image: "\ue363",
			"image-filled": "\ue877",
			"location-filled": "\ue333",
			location: "\ue303",
			"plus-filled": "\ue439",
			plus: "\ue409",
			plusempty: "\ue468",
			"help-filled": "\ue535",
			help: "\ue505",
			"navigate-filled": "\ue884",
			navigate: "\ue501",
			"mic-slash-filled": "\ue892",
			search: "\ue466",
			settings: "\ue560",
			sound: "\ue590",
			"sound-filled": "\ue8a1",
			"spinner-cycle": "\ue465",
			"download-filled": "\ue8a4",
			"personadd-filled": "\ue132",
			"videocam-filled": "\ue8af",
			personadd: "\ue102",
			upload: "\ue402",
			"upload-filled": "\ue8b1",
			starhalf: "\ue463",
			"star-filled": "\ue438",
			star: "\ue408",
			trash: "\ue401",
			"phone-filled": "\ue230",
			compose: "\ue400",
			videocam: "\ue300",
			"trash-filled": "\ue8dc",
			download: "\ue403",
			"chatbubble-filled": "\ue232",
			chatbubble: "\ue202",
			"cloud-download": "\ue8e4",
			"cloud-upload-filled": "\ue8e5",
			"cloud-upload": "\ue8e6",
			"cloud-download-filled": "\ue8e9",
			headphones: "\ue8bf",
			shop: "\ue609",
		};
	},
	2: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(3),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	237: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(238),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	238: function (A, B, t) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e = {
			name: "UniFav",
			props: {
				star: { type: [Boolean, String], default: !0 },
				bgColor: { type: String, default: "#eeeeee" },
				fgColor: { type: String, default: "#666666" },
				bgColorChecked: { type: String, default: "#007aff" },
				fgColorChecked: { type: String, default: "#FFFFFF" },
				circle: { type: [Boolean, String], default: !1 },
				checked: { type: Boolean, default: !1 },
				contentText: {
					type: Object,
					default: function () {
						return { contentDefault: "\u6536\u85cf", contentFav: "\u5df2\u6536\u85cf" };
					},
				},
			},
			watch: {
				checked: function () {
					uni.report && (this.checked ? uni.report("\u6536\u85cf", "\u6536\u85cf") : uni.report("\u53d6\u6d88\u6536\u85cf", "\u53d6\u6d88\u6536\u85cf"));
				},
			},
			methods: {
				onClick: function () {
					this.$emit("click");
				},
			},
		};
		B.default = e;
	},
	239: function (A, B) {
		A.exports = {
			".uni-fav": {
				"": {
					flexDirection: ["row", 0, 0, 0],
					alignItems: ["center", 0, 0, 0],
					justifyContent: ["center", 0, 0, 0],
					width: ["60", 0, 0, 0],
					height: ["25", 0, 0, 0],
					lineHeight: ["25", 0, 0, 0],
					textAlign: ["center", 0, 0, 0],
					borderRadius: ["3", 0, 0, 0],
				},
			},
			".uni-fav--circle": { "": { borderRadius: ["30", 0, 0, 1] } },
			".uni-fav-star": {
				"": {
					height: ["25", 0, 0, 2],
					lineHeight: ["24", 0, 0, 2],
					marginRight: ["3", 0, 0, 2],
					alignItems: ["center", 0, 0, 2],
					justifyContent: ["center", 0, 0, 2],
				},
			},
			".uni-fav-text": {
				"": {
					height: ["25", 0, 0, 3],
					lineHeight: ["25", 0, 0, 3],
					alignItems: ["center", 0, 0, 3],
					justifyContent: ["center", 0, 0, 3],
					fontSize: ["14", 0, 0, 3],
				},
			},
			"@VERSION": 2,
		};
	},
	240: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(241),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	241: function (A, B, t) {
		"use strict";
		var e;
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var E = {
			name: "UniNavBar",
			components: { statusBar: ((e = t(683)) && e.__esModule ? e : { default: e }).default },
			props: {
				title: { type: String, default: "" },
				leftText: { type: String, default: "" },
				rightText: { type: String, default: "" },
				leftIcon: { type: String, default: "" },
				rightIcon: { type: String, default: "" },
				fixed: { type: [Boolean, String], default: !1 },
				color: { type: String, default: "#000000" },
				backgroundColor: { type: String, default: "#FFFFFF" },
				statusBar: { type: [Boolean, String], default: !1 },
				shadow: { type: [Boolean, String], default: !1 },
				border: { type: [Boolean, String], default: !0 },
			},
			mounted: function () {
				uni.report && "" !== this.title && uni.report("title", this.title);
			},
			methods: {
				onClickLeft: function () {
					this.$emit("clickLeft");
				},
				onClickRight: function () {
					this.$emit("clickRight");
				},
				onClickTitle: function () {
					this.$emit("clickTitle");
				},
			},
		};
		B.default = E;
	},
	242: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(243),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	243: function (A, B, t) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e = uni.getSystemInfoSync().statusBarHeight + "px",
			E = {
				name: "UniStatusBar",
				data: function () {
					return { statusBarHeight: e };
				},
			};
		B.default = E;
	},
	244: function (A, B) {
		A.exports = { ".uni-status-bar": { "": { height: ["20", 0, 0, 0] } }, "@VERSION": 2 };
	},
	245: function (A, B) {
		A.exports = {
			".uni-nav-bar-text": { "": { fontSize: ["34rpx", 0, 0, 0] } },
			".uni-nav-bar-right-text": { "": { fontSize: ["14", 0, 0, 1] } },
			".uni-navbar__content": { "": { position: ["relative", 0, 0, 2], backgroundColor: ["#ffffff", 0, 0, 2], overflow: ["hidden", 0, 0, 2] } },
			".uni-navbar__content_view": { "": { alignItems: ["center", 0, 0, 3], flexDirection: ["row", 0, 0, 3] } },
			".uni-navbar__header": { "": { flexDirection: ["row", 0, 0, 4], height: ["44", 0, 0, 4], lineHeight: ["44", 0, 0, 4], fontSize: ["16", 0, 0, 4] } },
			".uni-navbar__header-btns": {
				"": {
					flexWrap: ["nowrap", 0, 0, 5],
					width: ["120rpx", 0, 0, 5],
					paddingTop: [0, 0, 0, 5],
					paddingRight: ["6", 0, 0, 5],
					paddingBottom: [0, 0, 0, 5],
					paddingLeft: ["6", 0, 0, 5],
					justifyContent: ["center", 0, 0, 5],
					alignItems: ["center", 0, 0, 5],
				},
			},
			".uni-navbar__header-btns-left": { "": { width: ["150rpx", 0, 0, 6], justifyContent: ["flex-start", 0, 0, 6] } },
			".uni-navbar__header-btns-right": { "": { width: ["150rpx", 0, 0, 7], paddingRight: ["30rpx", 0, 0, 7], justifyContent: ["flex-end", 0, 0, 7] } },
			".uni-navbar__header-container": { "": { flex: [1, 0, 0, 8] } },
			".uni-navbar__header-container-inner": {
				"": { flex: [1, 0, 0, 9], alignItems: ["center", 0, 0, 9], justifyContent: ["center", 0, 0, 9], fontSize: ["14", 0, 0, 9] },
			},
			".uni-navbar__placeholder-view": { "": { height: ["44", 0, 0, 10] } },
			".uni-navbar--fixed": { "": { position: ["fixed", 0, 0, 11], zIndex: [998, 0, 0, 11] } },
			".uni-navbar--border": {
				"": { borderBottomWidth: ["1rpx", 0, 0, 13], borderBottomStyle: ["solid", 0, 0, 13], borderBottomColor: ["#e5e5e5", 0, 0, 13] },
			},
			"@VERSION": 2,
		};
	},
	246: function (A, B, t) {
		"use strict";
		var e = t(247),
			E = t.n(e);
		B.default = E.a;
	},
	247: function (A, B, t) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		B.default = {
			components: {},
			data: function () {
				return { checkList: [!1, !1, !1, !1, !1, !1], contentText: { contentDefault: "\u8ffd\u756a", contentFav: "\u5df2\u8ffd\u756a" } };
			},
			methods: {
				favClick: function (A) {
					(this.checkList[A] = !this.checkList[A]), this.$forceUpdate();
				},
			},
		};
	},
	248: function (A, B) {
		A.exports = {
			".example": { "": { paddingTop: [0, 0, 0, 2], paddingRight: ["15", 0, 0, 2], paddingBottom: [0, 0, 0, 2], paddingLeft: ["15", 0, 0, 2] } },
			".example-info": {
				"": {
					paddingTop: ["15", 0, 0, 3],
					paddingRight: ["15", 0, 0, 3],
					paddingBottom: ["15", 0, 0, 3],
					paddingLeft: ["15", 0, 0, 3],
					color: ["#3b4144", 0, 0, 3],
					backgroundColor: ["#ffffff", 0, 0, 3],
					fontSize: ["14", 0, 0, 3],
					lineHeight: ["20", 0, 0, 3],
				},
			},
			".example-info-text": { "": { fontSize: ["14", 0, 0, 4], lineHeight: ["20", 0, 0, 4], color: ["#3b4144", 0, 0, 4] } },
			".example-body": {
				"": {
					flexDirection: ["column", 0, 0, 5],
					paddingTop: ["15", 0, 0, 5],
					paddingRight: ["15", 0, 0, 5],
					paddingBottom: ["15", 0, 0, 5],
					paddingLeft: ["15", 0, 0, 5],
					backgroundColor: ["#ffffff", 0, 0, 5],
				},
			},
			".word-btn-white": { "": { fontSize: ["18", 0, 0, 6], color: ["#FFFFFF", 0, 0, 6] } },
			".word-btn": {
				"": {
					flexDirection: ["row", 0, 0, 7],
					alignItems: ["center", 0, 0, 7],
					justifyContent: ["center", 0, 0, 7],
					borderRadius: ["6", 0, 0, 7],
					height: ["48", 0, 0, 7],
					marginTop: ["15", 0, 0, 7],
					marginRight: ["15", 0, 0, 7],
					marginBottom: ["15", 0, 0, 7],
					marginLeft: ["15", 0, 0, 7],
					backgroundColor: ["#007AFF", 0, 0, 7],
				},
			},
			".word-btn--hover": { "": { backgroundColor: ["#4ca2ff", 0, 0, 8] } },
			".favBtn": { "": { marginTop: [0, 0, 0, 9], marginRight: ["20rpx", 0, 0, 9], marginBottom: ["20rpx", 0, 0, 9], marginLeft: [0, 0, 0, 9] } },
			".example-body-fullWidth": {
				"": { paddingTop: ["32rpx", 0, 0, 10], paddingRight: [0, 0, 0, 10], paddingBottom: ["32rpx", 0, 0, 10], paddingLeft: [0, 0, 0, 10] },
			},
			".example-body-first": { "": { flexDirection: ["row", 0, 0, 11], justifyContent: ["flex-start", 0, 0, 11] } },
			"@VERSION": 2,
		};
	},
	3: function (A, B, t) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e = {
			name: "UniSection",
			props: { type: { type: String, default: "" }, title: { type: String, default: "" }, subTitle: { type: String, default: "" } },
			data: function () {
				return {};
			},
			watch: {
				title: function (A) {
					uni.report && "" !== A && uni.report("title", A);
				},
			},
			methods: {
				onClick: function () {
					this.$emit("click");
				},
			},
		};
		B.default = e;
	},
	4: function (A, B) {
		A.exports = {
			".uni-section": {
				"": {
					position: ["relative", 0, 0, 0],
					marginTop: ["10", 0, 0, 0],
					flexDirection: ["row", 0, 0, 0],
					alignItems: ["center", 0, 0, 0],
					paddingTop: [0, 0, 0, 0],
					paddingRight: ["10", 0, 0, 0],
					paddingBottom: [0, 0, 0, 0],
					paddingLeft: ["10", 0, 0, 0],
					height: ["50", 0, 0, 0],
					backgroundColor: ["#f8f8f8", 0, 0, 0],
					fontWeight: ["normal", 0, 0, 0],
				},
			},
			".uni-section__head": {
				"": { flexDirection: ["row", 0, 0, 1], justifyContent: ["center", 0, 0, 1], alignItems: ["center", 0, 0, 1], marginRight: ["10", 0, 0, 1] },
			},
			".line": { "": { height: ["15", 0, 0, 2], backgroundColor: ["#c0c0c0", 0, 0, 2], borderRadius: ["5", 0, 0, 2], width: ["3", 0, 0, 2] } },
			".circle": {
				"": {
					width: ["8", 0, 0, 3],
					height: ["8", 0, 0, 3],
					borderTopRightRadius: ["50", 0, 0, 3],
					borderTopLeftRadius: ["50", 0, 0, 3],
					borderBottomLeftRadius: ["50", 0, 0, 3],
					borderBottomRightRadius: ["50", 0, 0, 3],
					backgroundColor: ["#c0c0c0", 0, 0, 3],
				},
			},
			".uni-section__content": { "": { flexDirection: ["column", 0, 0, 4], flex: [1, 0, 0, 4], color: ["#333333", 0, 0, 4] } },
			".uni-section__content-title": { "": { fontSize: ["14", 0, 0, 5], color: ["#333333", 0, 0, 5] } },
			".distraction": { "": { flexDirection: ["row", 0, 0, 6], alignItems: ["center", 0, 0, 6] } },
			".uni-section__content-sub": { "": { fontSize: ["12", 0, 0, 7], color: ["#999999", 0, 0, 7] } },
			"@VERSION": 2,
		};
	},
	485: function (A, B, t) {
		"use strict";
		t.d(B, "b", function () {
			return E;
		}),
			t.d(B, "c", function () {
				return c;
			}),
			t.d(B, "a", function () {
				return e;
			});
		var e = { uniSection: t(13).default, uniFav: t(681).default, uniNavBar: t(682).default },
			E = function () {
				var A = this,
					B = A.$createElement,
					t = A._self._c || B;
				return t("scroll-view", { staticStyle: { flexDirection: "column" }, attrs: { scrollY: !0, showScrollbar: !0, enableBackToTop: !0, bubble: "true" } }, [
					t(
						"view",
						{ staticClass: ["example-fav"] },
						[
							t("uni-section", { attrs: { title: "\u57fa\u672c\u7528\u6cd5", type: "line" } }),
							t(
								"view",
								{ staticClass: ["example-body", "example-body-first"] },
								[
									t("uni-fav", {
										staticClass: ["favBtn"],
										attrs: { checked: A.checkList[0] },
										on: {
											click: function (B) {
												A.favClick(0);
											},
										},
									}),
									t("uni-fav", {
										staticClass: ["favBtn"],
										attrs: { checked: A.checkList[1], star: !1 },
										on: {
											click: function (B) {
												A.favClick(1);
											},
										},
									}),
									t("uni-fav", {
										staticClass: ["favBtn"],
										attrs: {
											checked: A.checkList[2],
											circle: !0,
											bgColor: "#dd524d",
											bgColorChecked: "#007aff",
											fgColor: "#ffffff",
											fgColorChecked: "#ffffff",
										},
										on: {
											click: function (B) {
												A.favClick(2);
											},
										},
									}),
									t("uni-fav", {
										staticClass: ["favBtn"],
										attrs: { checked: A.checkList[3], bgColor: "#f8f8f8", bgColorChecked: "#eeeeee", fgColor: "#333333", fgColorChecked: "#333333" },
										on: {
											click: function (B) {
												A.favClick(3);
											},
										},
									}),
								],
								1,
							),
							t("uni-section", { attrs: { title: "\u81ea\u5b9a\u4e49\u6587\u5b57", type: "line" } }),
							t(
								"view",
								{ staticClass: ["example-body"] },
								[
									t("uni-fav", {
										staticClass: ["favBtn"],
										attrs: { checked: A.checkList[4], contentText: A.contentText },
										on: {
											click: function (B) {
												A.favClick(4);
											},
										},
									}),
								],
								1,
							),
							t("uni-section", { attrs: { title: "\u5728\u81ea\u5b9a\u4e49\u5bfc\u822a\u680f\u4f7f\u7528", type: "line" } }),
							t(
								"view",
								{ staticClass: ["example-body", "example-body-fullWidth"] },
								[
									t(
										"uni-nav-bar",
										{
											staticStyle: { width: "100%" },
											attrs: { fixed: !1, leftIcon: "arrowleft", title: "\u6807\u9898", color: "#333333", backgroundColor: "#FFFFFF" },
										},
										[
											t(
												"block",
												{ attrs: { slot: "right" }, slot: "right" },
												[
													t("uni-fav", {
														staticClass: ["favBtn-nav"],
														attrs: { checked: A.checkList[5], circle: !0 },
														on: {
															click: function (B) {
																A.favClick(5);
															},
														},
													}),
												],
												1,
											),
										],
										1,
									),
								],
								1,
							),
						],
						1,
					),
				]);
			},
			c = [];
	},
	5: function (A, B, t) {
		"use strict";
		t.d(B, "b", function () {
			return e;
		}),
			t.d(B, "c", function () {
				return E;
			}),
			t.d(B, "a", function () {});
		var e = function () {
				var A = this,
					B = A.$createElement,
					t = A._self._c || B;
				return t(
					"view",
					{ staticClass: ["uni-section"], attrs: { nvue: !0 } },
					[
						A.type ? t("view", { staticClass: ["uni-section__head"] }, [t("view", { staticClass: ["uni-section__head-tag"], class: A.type })]) : A._e(),
						t("view", { staticClass: ["uni-section__content"] }, [
							t("u-text", { staticClass: ["uni-section__content-title"], class: { distraction: !A.subTitle }, appendAsTree: !0, attrs: { append: "tree" } }, [
								A._v(A._s(A.title)),
							]),
							A.subTitle
								? t("u-text", { staticClass: ["uni-section__content-sub"], appendAsTree: !0, attrs: { append: "tree" } }, [A._v(A._s(A.subTitle))])
								: A._e(),
						]),
						A._t("default"),
					],
					2,
				);
			},
			E = [];
	},
	503: function (A, B, t) {
		"use strict";
		t.d(B, "b", function () {
			return E;
		}),
			t.d(B, "c", function () {
				return c;
			}),
			t.d(B, "a", function () {
				return e;
			});
		var e = { uniIcons: t(16).default },
			E = function () {
				var A = this,
					B = A.$createElement,
					t = A._self._c || B;
				return t("view", { staticClass: ["uni-navbar"] }, [
					t(
						"view",
						{
							staticClass: ["uni-navbar__content"],
							class: { "uni-navbar--fixed": A.fixed, "uni-navbar--shadow": A.shadow, "uni-navbar--border": A.border },
							style: { "background-color": A.backgroundColor },
						},
						[
							A.statusBar ? t("status-bar") : A._e(),
							t("view", { staticClass: ["uni-navbar__header", "uni-navbar__content_view"], style: { color: A.color, backgroundColor: A.backgroundColor } }, [
								t(
									"view",
									{ staticClass: ["uni-navbar__header-btns", "uni-navbar__header-btns-left", "uni-navbar__content_view"], on: { click: A.onClickLeft } },
									[
										A.leftIcon.length
											? t(
													"view",
													{ staticClass: ["uni-navbar__content_view"] },
													[t("uni-icons", { attrs: { color: A.color, type: A.leftIcon, size: "24" } })],
													1,
											  )
											: A._e(),
										A.leftText.length
											? t(
													"view",
													{ staticClass: ["uni-navbar-btn-text", "uni-navbar__content_view"], class: { "uni-navbar-btn-icon-left": !A.leftIcon.length } },
													[t("u-text", { style: { color: A.color, fontSize: "14px" }, appendAsTree: !0, attrs: { append: "tree" } }, [A._v(A._s(A.leftText))])],
											  )
											: A._e(),
										A._t("left"),
									],
									2,
								),
								t(
									"view",
									{ staticClass: ["uni-navbar__header-container", "uni-navbar__content_view"], on: { click: A.onClickTitle } },
									[
										A.title.length
											? t("view", { staticClass: ["uni-navbar__header-container-inner", "uni-navbar__content_view"] }, [
													t("u-text", { staticClass: ["uni-nav-bar-text"], style: { color: A.color }, appendAsTree: !0, attrs: { append: "tree" } }, [
														A._v(A._s(A.title)),
													]),
											  ])
											: A._e(),
										A._t("default"),
									],
									2,
								),
								t(
									"view",
									{
										staticClass: ["uni-navbar__header-btns", "uni-navbar__content_view"],
										class: A.title.length ? "uni-navbar__header-btns-right" : "",
										on: { click: A.onClickRight },
									},
									[
										A.rightIcon.length
											? t(
													"view",
													{ staticClass: ["uni-navbar__content_view"] },
													[t("uni-icons", { attrs: { color: A.color, type: A.rightIcon, size: "24" } })],
													1,
											  )
											: A._e(),
										A.rightText.length && !A.rightIcon.length
											? t("view", { staticClass: ["uni-navbar-btn-text", "uni-navbar__content_view"] }, [
													t("u-text", { staticClass: ["uni-nav-bar-right-text"], appendAsTree: !0, attrs: { append: "tree" } }, [A._v(A._s(A.rightText))]),
											  ])
											: A._e(),
										A._t("right"),
									],
									2,
								),
							]),
						],
						1,
					),
					A.fixed
						? t(
								"view",
								{ staticClass: ["uni-navbar__placeholder"] },
								[A.statusBar ? t("status-bar") : A._e(), t("view", { staticClass: ["uni-navbar__placeholder-view"] })],
								1,
						  )
						: A._e(),
				]);
			},
			c = [];
	},
	504: function (A, B, t) {
		"use strict";
		t.d(B, "b", function () {
			return E;
		}),
			t.d(B, "c", function () {
				return c;
			}),
			t.d(B, "a", function () {
				return e;
			});
		var e = { uniIcons: t(16).default },
			E = function () {
				var A = this,
					B = A.$createElement,
					t = A._self._c || B;
				return t(
					"view",
					{
						staticClass: ["uni-fav"],
						class: [!0 === A.circle || "true" === A.circle ? "uni-fav--circle" : ""],
						style: [{ backgroundColor: A.checked ? A.bgColorChecked : A.bgColor }],
						on: { click: A.onClick },
					},
					[
						A.checked || (!0 !== A.star && "true" !== A.star)
							? A._e()
							: t("uni-icons", {
									staticClass: ["uni-fav-star"],
									style: { color: A.checked ? A.fgColorChecked : A.fgColor },
									attrs: { color: A.fgColor, size: "14", type: "star-filled" },
							  }),
						t(
							"u-text",
							{ staticClass: ["uni-fav-text"], style: { color: A.checked ? A.fgColorChecked : A.fgColor }, appendAsTree: !0, attrs: { append: "tree" } },
							[A._v(A._s(A.checked ? A.contentText.contentFav : A.contentText.contentDefault))],
						),
					],
					1,
				);
			},
			c = [];
	},
	531: function (A, B, t) {
		"use strict";
		t.d(B, "b", function () {
			return e;
		}),
			t.d(B, "c", function () {
				return E;
			}),
			t.d(B, "a", function () {});
		var e = function () {
				var A = this.$createElement;
				return (this._self._c || A)("view", { staticClass: ["uni-status-bar"], style: { height: this.statusBarHeight } }, [this._t("default")], 2);
			},
			E = [];
	},
	578: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(239),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	579: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(244),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	580: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(245),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	581: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(248),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	6: function (A, B, t) {
		(Vue.prototype.__$appStyle__ = {}), Vue.prototype.__merge_style && Vue.prototype.__merge_style(t(7).default, Vue.prototype.__$appStyle__);
	},
	680: function (A, B, t) {
		"use strict";
		t.r(B);
		t(6);
		var e = t(92);
		"undefined" == typeof Promise ||
			Promise.prototype.finally ||
			(Promise.prototype.finally = function (A) {
				var B = this.constructor;
				return this.then(
					function (t) {
						return B.resolve(A()).then(function () {
							return t;
						});
					},
					function (t) {
						return B.resolve(A()).then(function () {
							throw t;
						});
					},
				);
			}),
			(e.default.mpType = "page"),
			(e.default.route = "pages/extUI/fav/fav"),
			(e.default.el = "#root"),
			new Vue(e.default);
	},
	681: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(504),
			E = t(237);
		for (var c in E)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return E[A];
					});
				})(c);
		var i = t(0);
		var g = Object(i.a)(E.default, e.b, e.c, !1, null, "5735c9e1", "658f36b3", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(t(578).default, this.options.style) : Object.assign(this.options.style, t(578).default);
		}).call(g),
			(B.default = g.exports);
	},
	682: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(503),
			E = t(240);
		for (var c in E)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return E[A];
					});
				})(c);
		var i = t(0);
		var g = Object(i.a)(E.default, e.b, e.c, !1, null, "c9d7fd62", "2f654f4f", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(t(580).default, this.options.style) : Object.assign(this.options.style, t(580).default);
		}).call(g),
			(B.default = g.exports);
	},
	683: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(531),
			E = t(242);
		for (var c in E)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return E[A];
					});
				})(c);
		var i = t(0);
		var g = Object(i.a)(E.default, e.b, e.c, !1, null, "6284e93d", "6eb08ac2", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(t(579).default, this.options.style) : Object.assign(this.options.style, t(579).default);
		}).call(g),
			(B.default = g.exports);
	},
	7: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(1),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	8: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(4),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	9: function (A, B, t) {
		"use strict";
		t.r(B);
		var e = t(10),
			E = t.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					t.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = E.a;
	},
	92: function (A, B, t) {
		"use strict";
		var e = t(485),
			E = t(246),
			c = t(0);
		var i = Object(c.a)(E.default, e.b, e.c, !1, null, null, "216e2b48", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(t(581).default, this.options.style) : Object.assign(this.options.style, t(581).default);
		}).call(i),
			(B.default = i.exports);
	},
});
