!(function (A) {
	var B = {};
	function E(e) {
		if (B[e]) return B[e].exports;
		var t = (B[e] = { i: e, l: !1, exports: {} });
		return A[e].call(t.exports, t, t.exports, E), (t.l = !0), t.exports;
	}
	(E.m = A),
		(E.c = B),
		(E.d = function (A, B, e) {
			E.o(A, B) || Object.defineProperty(A, B, { enumerable: !0, get: e });
		}),
		(E.r = function (A) {
			"undefined" != typeof Symbol && Symbol.toStringTag && Object.defineProperty(A, Symbol.toStringTag, { value: "Module" }),
				Object.defineProperty(A, "__esModule", { value: !0 });
		}),
		(E.t = function (A, B) {
			if ((1 & B && (A = E(A)), 8 & B)) return A;
			if (4 & B && "object" == typeof A && A && A.__esModule) return A;
			var e = Object.create(null);
			if ((E.r(e), Object.defineProperty(e, "default", { enumerable: !0, value: A }), 2 & B && "string" != typeof A))
				for (var t in A)
					E.d(
						e,
						t,
						function (B) {
							return A[B];
						}.bind(null, t),
					);
			return e;
		}),
		(E.n = function (A) {
			var B =
				A && A.__esModule
					? function () {
							return A.default;
					  }
					: function () {
							return A;
					  };
			return E.d(B, "a", B), B;
		}),
		(E.o = function (A, B) {
			return Object.prototype.hasOwnProperty.call(A, B);
		}),
		(E.p = ""),
		E((E.s = 701));
})({
	0: function (A, B, E) {
		"use strict";
		function e(A, B, E, e, t, i, g, Q, o, n) {
			var c,
				w = "function" == typeof A ? A.options : A;
			if (o) {
				w.components || (w.components = {});
				var s = Object.prototype.hasOwnProperty;
				for (var l in o) s.call(o, l) && !s.call(w.components, l) && (w.components[l] = o[l]);
			}
			if (
				(n &&
					((n.beforeCreate || (n.beforeCreate = [])).unshift(function () {
						this[n.__module] = this;
					}),
					(w.mixins || (w.mixins = [])).push(n)),
				B && ((w.render = B), (w.staticRenderFns = E), (w._compiled = !0)),
				e && (w.functional = !0),
				i && (w._scopeId = "data-v-" + i),
				g
					? ((c = function (A) {
							(A = A || (this.$vnode && this.$vnode.ssrContext) || (this.parent && this.parent.$vnode && this.parent.$vnode.ssrContext)) ||
								"undefined" == typeof __VUE_SSR_CONTEXT__ ||
								(A = __VUE_SSR_CONTEXT__),
								t && t.call(this, A),
								A && A._registeredComponents && A._registeredComponents.add(g);
					  }),
					  (w._ssrRegister = c))
					: t &&
					  (c = Q
							? function () {
									t.call(this, this.$root.$options.shadowRoot);
							  }
							: t),
				c)
			)
				if (w.functional) {
					w._injectStyles = c;
					var a = w.render;
					w.render = function (A, B) {
						return c.call(B), a(A, B);
					};
				} else {
					var r = w.beforeCreate;
					w.beforeCreate = r ? [].concat(r, c) : [c];
				}
			return { exports: A, options: w };
		}
		E.d(B, "a", function () {
			return e;
		});
	},
	1: function (A, B) {
		A.exports = { "@VERSION": 2 };
	},
	10: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e,
			t = (e = E(17)) && e.__esModule ? e : { default: e };
		weex
			.requireModule("dom")
			.addRule("fontFace", {
				fontFamily: "uniicons",
				src: "url('data:font/truetype;charset=utf-8;base64,')",
			});
		var i = {
			name: "UniIcons",
			props: {
				type: { type: String, default: "" },
				color: { type: String, default: "#333333" },
				size: { type: [Number, String], default: 16 },
				customIcons: { type: String, default: "" },
			},
			data: function () {
				return { icons: t.default };
			},
			methods: {
				_onClick: function () {
					this.$emit("click");
				},
			},
		};
		B.default = i;
	},
	11: function (A, B) {
		A.exports = {
			".uni-icons": { "": { fontFamily: ["uniicons", 0, 0, 0], textDecoration: ["none", 0, 0, 0], textAlign: ["center", 0, 0, 0] } },
			"@VERSION": 2,
		};
	},
	12: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return e;
		}),
			E.d(B, "c", function () {
				return t;
			}),
			E.d(B, "a", function () {});
		var e = function () {
				var A = this,
					B = A.$createElement;
				return (A._self._c || B)(
					"u-text",
					{
						staticClass: ["uni-icons"],
						class: [A.customIcons, A.customIcons ? A.type : ""],
						style: { color: A.color, "font-size": A.size + "px" },
						appendAsTree: !0,
						attrs: { append: "tree" },
						on: { click: A._onClick },
					},
					[A._v(A._s(A.icons[A.type]))],
				);
			},
			t = [];
	},
	13: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(5),
			t = E(2);
		for (var i in t)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return t[A];
					});
				})(i);
		var g = E(0);
		var Q = Object(g.a)(t.default, e.b, e.c, !1, null, "168bca6c", "39eb9647", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(8).default, this.options.style) : Object.assign(this.options.style, E(8).default);
		}).call(Q),
			(B.default = Q.exports);
	},
	15: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(11),
			t = E.n(e);
		for (var i in e)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(i);
		B.default = t.a;
	},
	16: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(12),
			t = E(9);
		for (var i in t)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return t[A];
					});
				})(i);
		var g = E(0);
		var Q = Object(g.a)(t.default, e.b, e.c, !1, null, "041e7dc0", "67d6d51e", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(15).default, this.options.style) : Object.assign(this.options.style, E(15).default);
		}).call(Q),
			(B.default = Q.exports);
	},
	17: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		B.default = {
			pulldown: "\ue588",
			refreshempty: "\ue461",
			back: "\ue471",
			forward: "\ue470",
			more: "\ue507",
			"more-filled": "\ue537",
			scan: "\ue612",
			qq: "\ue264",
			weibo: "\ue260",
			weixin: "\ue261",
			pengyouquan: "\ue262",
			loop: "\ue565",
			refresh: "\ue407",
			"refresh-filled": "\ue437",
			arrowthindown: "\ue585",
			arrowthinleft: "\ue586",
			arrowthinright: "\ue587",
			arrowthinup: "\ue584",
			"undo-filled": "\ue7d6",
			undo: "\ue406",
			redo: "\ue405",
			"redo-filled": "\ue7d9",
			bars: "\ue563",
			chatboxes: "\ue203",
			camera: "\ue301",
			"chatboxes-filled": "\ue233",
			"camera-filled": "\ue7ef",
			"cart-filled": "\ue7f4",
			cart: "\ue7f5",
			"checkbox-filled": "\ue442",
			checkbox: "\ue7fa",
			arrowleft: "\ue582",
			arrowdown: "\ue581",
			arrowright: "\ue583",
			"smallcircle-filled": "\ue801",
			arrowup: "\ue580",
			circle: "\ue411",
			"eye-filled": "\ue568",
			"eye-slash-filled": "\ue822",
			"eye-slash": "\ue823",
			eye: "\ue824",
			"flag-filled": "\ue825",
			flag: "\ue508",
			"gear-filled": "\ue532",
			reload: "\ue462",
			gear: "\ue502",
			"hand-thumbsdown-filled": "\ue83b",
			"hand-thumbsdown": "\ue83c",
			"hand-thumbsup-filled": "\ue83d",
			"heart-filled": "\ue83e",
			"hand-thumbsup": "\ue83f",
			heart: "\ue840",
			home: "\ue500",
			info: "\ue504",
			"home-filled": "\ue530",
			"info-filled": "\ue534",
			"circle-filled": "\ue441",
			"chat-filled": "\ue847",
			chat: "\ue263",
			"mail-open-filled": "\ue84d",
			"email-filled": "\ue231",
			"mail-open": "\ue84e",
			email: "\ue201",
			checkmarkempty: "\ue472",
			list: "\ue562",
			"locked-filled": "\ue856",
			locked: "\ue506",
			"map-filled": "\ue85c",
			"map-pin": "\ue85e",
			"map-pin-ellipse": "\ue864",
			map: "\ue364",
			"minus-filled": "\ue440",
			"mic-filled": "\ue332",
			minus: "\ue410",
			micoff: "\ue360",
			mic: "\ue302",
			clear: "\ue434",
			smallcircle: "\ue868",
			close: "\ue404",
			closeempty: "\ue460",
			paperclip: "\ue567",
			paperplane: "\ue503",
			"paperplane-filled": "\ue86e",
			"person-filled": "\ue131",
			"contact-filled": "\ue130",
			person: "\ue101",
			contact: "\ue100",
			"images-filled": "\ue87a",
			phone: "\ue200",
			images: "\ue87b",
			image: "\ue363",
			"image-filled": "\ue877",
			"location-filled": "\ue333",
			location: "\ue303",
			"plus-filled": "\ue439",
			plus: "\ue409",
			plusempty: "\ue468",
			"help-filled": "\ue535",
			help: "\ue505",
			"navigate-filled": "\ue884",
			navigate: "\ue501",
			"mic-slash-filled": "\ue892",
			search: "\ue466",
			settings: "\ue560",
			sound: "\ue590",
			"sound-filled": "\ue8a1",
			"spinner-cycle": "\ue465",
			"download-filled": "\ue8a4",
			"personadd-filled": "\ue132",
			"videocam-filled": "\ue8af",
			personadd: "\ue102",
			upload: "\ue402",
			"upload-filled": "\ue8b1",
			starhalf: "\ue463",
			"star-filled": "\ue438",
			star: "\ue408",
			trash: "\ue401",
			"phone-filled": "\ue230",
			compose: "\ue400",
			videocam: "\ue300",
			"trash-filled": "\ue8dc",
			download: "\ue403",
			"chatbubble-filled": "\ue232",
			chatbubble: "\ue202",
			"cloud-download": "\ue8e4",
			"cloud-upload-filled": "\ue8e5",
			"cloud-upload": "\ue8e6",
			"cloud-download-filled": "\ue8e9",
			headphones: "\ue8bf",
			shop: "\ue609",
		};
	},
	2: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(3),
			t = E.n(e);
		for (var i in e)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(i);
		B.default = t.a;
	},
	286: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(287),
			t = E.n(e);
		for (var i in e)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(i);
		B.default = t.a;
	},
	287: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e = weex.requireModule("dom"),
			t = weex.requireModule("animation"),
			i = {
				name: "UniNoticeBar",
				props: {
					text: { type: String, default: "" },
					moreText: { type: String, default: "" },
					backgroundColor: { type: String, default: "#fffbe8" },
					speed: { type: Number, default: 100 },
					color: { type: String, default: "#de8c17" },
					moreColor: { type: String, default: "#999999" },
					single: { type: [Boolean, String], default: !1 },
					scrollable: { type: [Boolean, String], default: !1 },
					showIcon: { type: [Boolean, String], default: !1 },
					showGetMore: { type: [Boolean, String], default: !1 },
					showClose: { type: [Boolean, String], default: !1 },
				},
				data: function () {
					return {
						textWidth: 0,
						boxWidth: 0,
						wrapWidth: "",
						webviewHide: !1,
						stopAnimation: !1,
						elId: "Uni_".concat(Math.ceil(1e6 * Math.random()).toString(36)),
						elIdBox: "Uni_".concat(Math.ceil(1e6 * Math.random()).toString(36)),
						show: !0,
						animationDuration: "none",
						animationPlayState: "paused",
						animationDelay: "0s",
					};
				},
				mounted: function () {
					var A = this,
						B = getCurrentPages(),
						E = B[B.length - 1].$getAppWebview();
					E.addEventListener("hide", function () {
						A.webviewHide = !0;
					}),
						E.addEventListener("show", function () {
							A.webviewHide = !1;
						}),
						this.$nextTick(function () {
							A.initSize();
						});
				},
				beforeDestroy: function () {
					this.stopAnimation = !0;
				},
				methods: {
					initSize: function () {
						var A = this;
						this.scrollable &&
							e.getComponentRect(this.$refs.animationEle, function (B) {
								var E = uni.getSystemInfoSync().windowWidth;
								(A.textWidth = B.size.width),
									t.transition(
										A.$refs.animationEle,
										{ styles: { transform: "translateX(-".concat(E, "px)") }, duration: 0, timingFunction: "linear", delay: 0 },
										function () {
											A.stopAnimation ||
												t.transition(
													A.$refs.animationEle,
													{
														styles: { transform: "translateX(-".concat(A.textWidth, "px)") },
														timingFunction: "linear",
														duration: ((A.textWidth - E) / A.speed) * 1e3,
														delay: 1e3,
													},
													function () {
														A.stopAnimation || A.loopAnimation();
													},
												);
										},
									);
							}),
							this.scrollable ||
								(!this.single && !this.moreText) ||
								e.getComponentRect(this.$refs.textBox, function (B) {
									A.wrapWidth = B.size.width;
								});
					},
					loopAnimation: function () {
						var A = this;
						t.transition(this.$refs.animationEle, { styles: { transform: "translateX(0px)" }, duration: 0 }, function () {
							A.stopAnimation ||
								t.transition(
									A.$refs.animationEle,
									{
										styles: { transform: "translateX(-".concat(A.textWidth, "px)") },
										duration: (A.textWidth / A.speed) * 1e3,
										timingFunction: "linear",
										delay: 0,
									},
									function () {
										A.stopAnimation || A.loopAnimation();
									},
								);
						});
					},
					clickMore: function () {
						this.$emit("getmore");
					},
					close: function () {
						(this.show = !1), this.$emit("close");
					},
					onClick: function () {
						this.$emit("click");
					},
				},
			};
		B.default = i;
	},
	288: function (A, B) {
		A.exports = {
			".uni-noticebar": {
				"": {
					flexDirection: ["row", 0, 0, 0],
					alignItems: ["center", 0, 0, 0],
					paddingTop: ["6", 0, 0, 0],
					paddingRight: ["12", 0, 0, 0],
					paddingBottom: ["6", 0, 0, 0],
					paddingLeft: ["12", 0, 0, 0],
					marginBottom: ["10", 0, 0, 0],
				},
			},
			".uni-noticebar-close": { "": { marginRight: ["5", 0, 0, 2] } },
			".uni-noticebar-icon": { "": { marginRight: ["5", 0, 0, 3] } },
			".uni-noticebar__content-wrapper": { "": { flex: [1, 0, 0, 4], flexDirection: ["column", 0, 0, 4], overflow: ["hidden", 0, 0, 4] } },
			".uni-noticebar__content-wrapper--single": { "": { flexDirection: ["row", 0, 0, 6] } },
			".uni-noticebar__content-wrapper--scrollable": { "": { flexDirection: ["row", 0, 0, 6] } },
			".uni-noticebar__content--scrollable": { "": { flex: [0, 0, 0, 7] } },
			".uni-noticebar__content-text": { "": { fontSize: ["14", 0, 0, 9], lineHeight: ["18", 0, 0, 9] } },
			".uni-noticebar__content-text--single": { "": { lines: [1, 0, 0, 10], overflow: ["hidden", 0, 0, 10], textOverflow: ["ellipsis", 0, 0, 10] } },
			".uni-noticebar__content-text--scrollable": { "": { lines: [1, 0, 0, 11], paddingLeft: ["750rpx", 0, 0, 11] } },
			".uni-noticebar__more": {
				"": { flexDirection: ["row", 0, 0, 12], flexWrap: ["nowrap", 0, 0, 12], alignItems: ["center", 0, 0, 12], paddingLeft: ["5", 0, 0, 12] },
			},
			".uni-noticebar__more-text": { "": { fontSize: ["14", 0, 0, 13] } },
			"@VERSION": 2,
		};
	},
	289: function (A, B, E) {
		"use strict";
		var e = E(290),
			t = E.n(e);
		B.default = t.a;
	},
	290: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e = {
			components: {},
			data: function () {
				return {};
			},
			created: function () {},
			methods: {
				getMore: function () {
					uni.showToast({ title: "\u70b9\u51fb\u67e5\u770b\u66f4\u591a", icon: "none" });
				},
			},
		};
		B.default = e;
	},
	291: function (A, B) {
		A.exports = {
			".example": { "": { paddingTop: [0, 0, 0, 2], paddingRight: ["15", 0, 0, 2], paddingBottom: [0, 0, 0, 2], paddingLeft: ["15", 0, 0, 2] } },
			".example-info": {
				"": {
					paddingTop: ["15", 0, 0, 3],
					paddingRight: ["15", 0, 0, 3],
					paddingBottom: ["15", 0, 0, 3],
					paddingLeft: ["15", 0, 0, 3],
					color: ["#3b4144", 0, 0, 3],
					backgroundColor: ["#ffffff", 0, 0, 3],
					fontSize: ["14", 0, 0, 3],
					lineHeight: ["20", 0, 0, 3],
				},
			},
			".example-info-text": { "": { fontSize: ["14", 0, 0, 4], lineHeight: ["20", 0, 0, 4], color: ["#3b4144", 0, 0, 4] } },
			".example-body": {
				"": {
					flexDirection: ["column", 0, 0, 9],
					paddingTop: ["20rpx", 0, 0, 9],
					paddingRight: [0, 0, 0, 9],
					paddingBottom: ["5rpx", 0, 0, 9],
					paddingLeft: [0, 0, 0, 9],
					backgroundColor: ["#ffffff", 0, 0, 5],
				},
			},
			".word-btn-white": { "": { fontSize: ["18", 0, 0, 6], color: ["#FFFFFF", 0, 0, 6] } },
			".word-btn": {
				"": {
					flexDirection: ["row", 0, 0, 7],
					alignItems: ["center", 0, 0, 7],
					justifyContent: ["center", 0, 0, 7],
					borderRadius: ["6", 0, 0, 7],
					height: ["48", 0, 0, 7],
					marginTop: ["15", 0, 0, 7],
					marginRight: ["15", 0, 0, 7],
					marginBottom: ["15", 0, 0, 7],
					marginLeft: ["15", 0, 0, 7],
					backgroundColor: ["#007AFF", 0, 0, 7],
				},
			},
			".word-btn--hover": { "": { backgroundColor: ["#4ca2ff", 0, 0, 8] } },
			"@VERSION": 2,
		};
	},
	3: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e = {
			name: "UniSection",
			props: { type: { type: String, default: "" }, title: { type: String, default: "" }, subTitle: { type: String, default: "" } },
			data: function () {
				return {};
			},
			watch: {
				title: function (A) {
					uni.report && "" !== A && uni.report("title", A);
				},
			},
			methods: {
				onClick: function () {
					this.$emit("click");
				},
			},
		};
		B.default = e;
	},
	4: function (A, B) {
		A.exports = {
			".uni-section": {
				"": {
					position: ["relative", 0, 0, 0],
					marginTop: ["10", 0, 0, 0],
					flexDirection: ["row", 0, 0, 0],
					alignItems: ["center", 0, 0, 0],
					paddingTop: [0, 0, 0, 0],
					paddingRight: ["10", 0, 0, 0],
					paddingBottom: [0, 0, 0, 0],
					paddingLeft: ["10", 0, 0, 0],
					height: ["50", 0, 0, 0],
					backgroundColor: ["#f8f8f8", 0, 0, 0],
					fontWeight: ["normal", 0, 0, 0],
				},
			},
			".uni-section__head": {
				"": { flexDirection: ["row", 0, 0, 1], justifyContent: ["center", 0, 0, 1], alignItems: ["center", 0, 0, 1], marginRight: ["10", 0, 0, 1] },
			},
			".line": { "": { height: ["15", 0, 0, 2], backgroundColor: ["#c0c0c0", 0, 0, 2], borderRadius: ["5", 0, 0, 2], width: ["3", 0, 0, 2] } },
			".circle": {
				"": {
					width: ["8", 0, 0, 3],
					height: ["8", 0, 0, 3],
					borderTopRightRadius: ["50", 0, 0, 3],
					borderTopLeftRadius: ["50", 0, 0, 3],
					borderBottomLeftRadius: ["50", 0, 0, 3],
					borderBottomRightRadius: ["50", 0, 0, 3],
					backgroundColor: ["#c0c0c0", 0, 0, 3],
				},
			},
			".uni-section__content": { "": { flexDirection: ["column", 0, 0, 4], flex: [1, 0, 0, 4], color: ["#333333", 0, 0, 4] } },
			".uni-section__content-title": { "": { fontSize: ["14", 0, 0, 5], color: ["#333333", 0, 0, 5] } },
			".distraction": { "": { flexDirection: ["row", 0, 0, 6], alignItems: ["center", 0, 0, 6] } },
			".uni-section__content-sub": { "": { fontSize: ["12", 0, 0, 7], color: ["#999999", 0, 0, 7] } },
			"@VERSION": 2,
		};
	},
	483: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return t;
		}),
			E.d(B, "c", function () {
				return i;
			}),
			E.d(B, "a", function () {
				return e;
			});
		var e = { uniSection: E(13).default, uniNoticeBar: E(702).default },
			t = function () {
				var A = this.$createElement,
					B = this._self._c || A;
				return B("scroll-view", { staticStyle: { flexDirection: "column" }, attrs: { scrollY: !0, showScrollbar: !0, enableBackToTop: !0, bubble: "true" } }, [
					B(
						"view",
						{ staticClass: ["fixforpc-window"] },
						[
							B("u-text", { staticClass: ["example-info"], appendAsTree: !0, attrs: { append: "tree" } }, [
								this._v(
									"\u901a\u544a\u680f\u7ec4\u4ef6\u591a\u7528\u4e8e\u7cfb\u7edf\u901a\u77e5\uff0c\u5e7f\u544a\u901a\u77e5\u7b49\u573a\u666f\uff0c\u53ef\u81ea\u5b9a\u4e49\u56fe\u6807\uff0c\u989c\u8272\uff0c\u5c55\u73b0\u65b9\u5f0f\u7b49\u3002",
								),
							]),
							B("uni-section", { attrs: { title: "\u57fa\u672c\u7528\u6cd5", type: "line" } }),
							B(
								"view",
								{ staticClass: ["example-body"] },
								[
									B("uni-notice-bar", {
										attrs: { single: !0, text: "[\u5355\u884c] uni-app 1.6\u53d1\u5e03\uff0c\u5f00\u53d1\u4e00\u6b21\u30017\u7aef\u8986\u76d6\uff01" },
									}),
									B("uni-notice-bar", {
										attrs: {
											text: "[\u591a\u884c] uni-app 1.6\u7248\u6b63\u5f0f\u53d1\u5e03\uff0c\u5f00\u53d1\u4e00\u6b21\uff0c\u540c\u65f6\u53d1\u5e03iOS\u3001Android\u3001H5\u3001\u5fae\u4fe1\u5c0f\u7a0b\u5e8f\u3001\u652f\u4ed8\u5b9d\u5c0f\u7a0b\u5e8f\u3001\u767e\u5ea6\u5c0f\u7a0b\u5e8f\u3001\u5934\u6761\u5c0f\u7a0b\u5e8f\u7b497\u5927\u5e73\u53f0\u3002",
										},
									}),
								],
								1,
							),
							B("uni-section", { attrs: { title: "\u52a0\u56fe\u6807", type: "line" } }),
							B(
								"view",
								{ staticClass: ["example-body"] },
								[
									B("uni-notice-bar", {
										attrs: {
											single: !0,
											showIcon: !0,
											text: "uni-app\u884c\u4e1a\u5cf0\u4f1a\u9891\u9891\u4eae\u76f8\uff0c\u5f00\u53d1\u8005\u53cd\u54cd\u70ed\u70c8!",
										},
									}),
									B("uni-notice-bar", {
										attrs: {
											showIcon: !0,
											text: "8\u670812\u65e5DCloud\u53d7\u9080\u53c2\u52a0iWEB\u5cf0\u4f1a\u540e\uff0c9\u670819\u65e5DCloud CEO \u738b\u5b89\u5728\u5343\u4eba\u5c0f\u7a0b\u5e8f\u5cf0\u4f1a\u2014\u2014\u89c1\u5b9e\u5927\u4f1a\uff0c\u505a\u4e86\u4e3b\u9898\u4e3a\u300aApp\u548c\u5c0f\u7a0b\u5e8f\uff0c\u9c7c\u4e0e\u718a\u638c\u5982\u4f55\u517c\u5f97\uff1f\u300b\u7684\u5206\u4eab\u3002",
										},
									}),
								],
								1,
							),
							B("uni-section", { attrs: { title: "\u6587\u5b57\u6eda\u52a8", type: "line" } }),
							B(
								"view",
								{ staticClass: ["example-body"] },
								[
									B("uni-notice-bar", {
										attrs: {
											scrollable: !0,
											single: !0,
											text: "uni-app\u884c\u4e1a\u5cf0\u4f1a\u9891\u9891\u4eae\u76f8\uff0c\u5f00\u53d1\u8005\u53cd\u54cd\u70ed\u70c8!",
										},
									}),
									B("uni-notice-bar", {
										attrs: {
											showIcon: !0,
											scrollable: !0,
											single: !0,
											text: "uni-app 1.6\u7248\u6b63\u5f0f\u53d1\u5e03\uff0c\u5f00\u53d1\u4e00\u6b21\uff0c\u540c\u65f6\u53d1\u5e03iOS\u3001Android\u3001H5\u3001\u5fae\u4fe1\u5c0f\u7a0b\u5e8f\u3001\u652f\u4ed8\u5b9d\u5c0f\u7a0b\u5e8f\u3001\u767e\u5ea6\u5c0f\u7a0b\u5e8f\u3001\u5934\u6761\u5c0f\u7a0b\u5e8f\u7b497\u5927\u5e73\u53f0\u3002",
										},
									}),
									B("uni-notice-bar", {
										attrs: {
											scrollable: !0,
											text: "8\u670812\u65e5DCloud\u53d7\u9080\u53c2\u52a0iWEB\u5cf0\u4f1a\u540e\uff0c9\u670819\u65e5DCloud CEO \u738b\u5b89\u5728\u5343\u4eba\u5c0f\u7a0b\u5e8f\u5cf0\u4f1a\u2014\u2014\u89c1\u5b9e\u5927\u4f1a\uff0c\u505a\u4e86\u4e3b\u9898\u4e3a\u300aApp\u548c\u5c0f\u7a0b\u5e8f\uff0c\u9c7c\u4e0e\u718a\u638c\u5982\u4f55\u517c\u5f97\uff1f\u300b\u7684\u5206\u4eab\u3002",
										},
									}),
								],
								1,
							),
							B("uni-section", { attrs: { title: "\u67e5\u770b\u66f4\u591a", type: "line" } }),
							B(
								"view",
								{ staticClass: ["example-body"] },
								[
									B("uni-notice-bar", {
										attrs: {
											showGetMore: !0,
											single: !0,
											text: "\u5e74\u672b\u5927\u793c\uff1auni-app1.4 \u65b0\u589e\u767e\u5ea6\u3001\u652f\u4ed8\u5b9d\u5c0f\u7a0b\u5e8f\u3002\u63d2\u4ef6\u5e02\u573a\u91cd\u78c5\u4e0a\u7ebf\uff01",
										},
										on: { getmore: this.getMore },
									}),
									B("uni-notice-bar", {
										attrs: {
											showGetMore: !0,
											showIcon: !0,
											single: !0,
											moreText: "\u67e5\u770b\u66f4\u591a",
											text: "\u5e74\u672b\u5927\u793c\uff1auni-app1.4 \u65b0\u589e\u767e\u5ea6\u3001\u652f\u4ed8\u5b9d\u5c0f\u7a0b\u5e8f\u3002\u63d2\u4ef6\u5e02\u573a\u91cd\u78c5\u4e0a\u7ebf\uff01",
										},
										on: { getmore: this.getMore },
									}),
								],
								1,
							),
							B("uni-section", { attrs: { title: "\u4fee\u6539\u989c\u8272", type: "line" } }),
							B(
								"view",
								{ staticClass: ["example-body"] },
								[
									B("uni-notice-bar", {
										attrs: {
											single: !0,
											moreText: "\u67e5\u770b\u66f4\u591a",
											color: "red",
											backgroundColor: "#eee",
											text: "uni-app 1.6\u7248\u6b63\u5f0f\u53d1\u5e03\uff0c\u5f00\u53d1\u4e00\u6b21\uff0c\u540c\u65f6\u53d1\u5e03iOS\u3001Android\u3001H5\u3001\u5fae\u4fe1\u5c0f\u7a0b\u5e8f\u3001\u652f\u4ed8\u5b9d\u5c0f\u7a0b\u5e8f\u3001\u767e\u5ea6\u5c0f\u7a0b\u5e8f\u3001\u5934\u6761\u5c0f\u7a0b\u5e8f\u7b497\u5927\u5e73\u53f0\u3002",
										},
									}),
									B("uni-notice-bar", {
										attrs: {
											single: !0,
											showGetMore: !0,
											moreText: "\u67e5\u770b\u66f4\u591a",
											moreColor: "blue",
											color: "red",
											backgroundColor: "#eee",
											text: "uni-app 1.6\u7248\u6b63\u5f0f\u53d1\u5e03\uff0c\u5f00\u53d1\u4e00\u6b21\uff0c\u540c\u65f6\u53d1\u5e03iOS\u3001Android\u3001H5\u3001\u5fae\u4fe1\u5c0f\u7a0b\u5e8f\u3001\u652f\u4ed8\u5b9d\u5c0f\u7a0b\u5e8f\u3001\u767e\u5ea6\u5c0f\u7a0b\u5e8f\u3001\u5934\u6761\u5c0f\u7a0b\u5e8f\u7b497\u5927\u5e73\u53f0\u3002",
										},
										on: { getmore: this.getMore },
									}),
									B("uni-notice-bar", {
										attrs: {
											showIcon: !0,
											color: "blue",
											text: "uni-app 1.6\u7248\u6b63\u5f0f\u53d1\u5e03\uff0c\u5f00\u53d1\u4e00\u6b21\uff0c\u540c\u65f6\u53d1\u5e03iOS\u3001Android\u3001H5\u3001\u5fae\u4fe1\u5c0f\u7a0b\u5e8f\u3001\u652f\u4ed8\u5b9d\u5c0f\u7a0b\u5e8f\u3001\u767e\u5ea6\u5c0f\u7a0b\u5e8f\u3001\u5934\u6761\u5c0f\u7a0b\u5e8f\u7b497\u5927\u5e73\u53f0\u3002",
										},
									}),
								],
								1,
							),
							B("uni-section", { attrs: { title: "\u5173\u95ed\u6309\u94ae", type: "line" } }),
							B(
								"view",
								{ staticClass: ["example-body"] },
								[
									B("uni-notice-bar", {
										attrs: {
											showClose: !0,
											single: !0,
											text: "HBuilderX 1.0\u6b63\u5f0f\u53d1\u5e03\uff01uni-app\u5b9e\u73b0\u91cc\u7a0b\u7891\u7a81\u7834\uff01",
										},
									}),
									B("uni-notice-bar", {
										attrs: {
											showClose: !0,
											showIcon: !0,
											moreText: "\u67e5\u770b\u66f4\u591a",
											text: "HBuilderX 1.0\u6b63\u5f0f\u53d1\u5e03\uff01uni-app\u5b9e\u73b0\u91cc\u7a0b\u7891\u7a81\u7834\uff01",
										},
									}),
									B("uni-notice-bar", {
										attrs: { showClose: !0, showIcon: !0, text: "uni-app 1.6\u53d1\u5e03\uff0c\u5f00\u53d1\u4e00\u6b21\u30017\u7aef\u8986\u76d6\uff01" },
									}),
									B("uni-notice-bar", {
										attrs: {
											showClose: !0,
											showIcon: !0,
											single: !0,
											text: "uni-app 1.6\u7248\u6b63\u5f0f\u53d1\u5e03\uff0c\u5f00\u53d1\u4e00\u6b21\uff0c\u540c\u65f6\u53d1\u5e03\u5230iOS\u3001Android\u3001H5\u3001\u5fae\u4fe1\u5c0f\u7a0b\u5e8f\u3001\u652f\u4ed8\u5b9d\u5c0f\u7a0b\u5e8f\u3001\u767e\u5ea6\u5c0f\u7a0b\u5e8f\u3001\u5934\u6761\u5c0f\u7a0b\u5e8f\u7b497\u5927\u5e73\u53f0\u3002",
										},
									}),
								],
								1,
							),
						],
						1,
					),
				]);
			},
			i = [];
	},
	5: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return e;
		}),
			E.d(B, "c", function () {
				return t;
			}),
			E.d(B, "a", function () {});
		var e = function () {
				var A = this,
					B = A.$createElement,
					E = A._self._c || B;
				return E(
					"view",
					{ staticClass: ["uni-section"], attrs: { nvue: !0 } },
					[
						A.type ? E("view", { staticClass: ["uni-section__head"] }, [E("view", { staticClass: ["uni-section__head-tag"], class: A.type })]) : A._e(),
						E("view", { staticClass: ["uni-section__content"] }, [
							E("u-text", { staticClass: ["uni-section__content-title"], class: { distraction: !A.subTitle }, appendAsTree: !0, attrs: { append: "tree" } }, [
								A._v(A._s(A.title)),
							]),
							A.subTitle
								? E("u-text", { staticClass: ["uni-section__content-sub"], appendAsTree: !0, attrs: { append: "tree" } }, [A._v(A._s(A.subTitle))])
								: A._e(),
						]),
						A._t("default"),
					],
					2,
				);
			},
			t = [];
	},
	511: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return t;
		}),
			E.d(B, "c", function () {
				return i;
			}),
			E.d(B, "a", function () {
				return e;
			});
		var e = { uniIcons: E(16).default },
			t = function () {
				var A = this,
					B = A.$createElement,
					E = A._self._c || B;
				return A.show
					? E(
							"view",
							{ staticClass: ["uni-noticebar"], style: { backgroundColor: A.backgroundColor }, on: { click: A.onClick } },
							[
								!0 === A.showClose || "true" === A.showClose
									? E("uni-icons", {
											staticClass: ["uni-noticebar-close", "uni-cursor-point"],
											attrs: { type: "closeempty", color: A.color, size: "12" },
											on: { click: A.close },
									  })
									: A._e(),
								!0 === A.showIcon || "true" === A.showIcon
									? E("uni-icons", { staticClass: ["uni-noticebar-icon"], attrs: { type: "sound", color: A.color, size: "14" } })
									: A._e(),
								E(
									"view",
									{
										ref: "textBox",
										staticClass: ["uni-noticebar__content-wrapper"],
										class: {
											"uni-noticebar__content-wrapper--scrollable": A.scrollable,
											"uni-noticebar__content-wrapper--single": !A.scrollable && (A.single || A.moreText),
										},
									},
									[
										E(
											"view",
											{
												staticClass: ["uni-noticebar__content"],
												class: {
													"uni-noticebar__content--scrollable": A.scrollable,
													"uni-noticebar__content--single": !A.scrollable && (A.single || A.moreText),
												},
												attrs: { id: A.elIdBox },
											},
											[
												E(
													"u-text",
													{
														ref: "animationEle",
														staticClass: ["uni-noticebar__content-text"],
														class: {
															"uni-noticebar__content-text--scrollable": A.scrollable,
															"uni-noticebar__content-text--single": !A.scrollable && (A.single || A.moreText),
														},
														style: {
															color: A.color,
															width: A.wrapWidth + "px",
															animationDuration: A.animationDuration,
															"-webkit-animationDuration": A.animationDuration,
															animationPlayState: A.webviewHide ? "paused" : A.animationPlayState,
															"-webkit-animationPlayState": A.webviewHide ? "paused" : A.animationPlayState,
															animationDelay: A.animationDelay,
															"-webkit-animationDelay": A.animationDelay,
														},
														appendAsTree: !0,
														attrs: { id: A.elId, append: "tree" },
													},
													[A._v(A._s(A.text))],
												),
											],
										),
									],
								),
								!0 === A.showGetMore || "true" === A.showGetMore
									? E(
											"view",
											{ staticClass: ["uni-noticebar__more", "uni-cursor-point"], on: { click: A.clickMore } },
											[
												A.moreText
													? E(
															"u-text",
															{ staticClass: ["uni-noticebar__more-text"], style: { color: A.moreColor }, appendAsTree: !0, attrs: { append: "tree" } },
															[A._v(A._s(A.moreText))],
													  )
													: A._e(),
												E("uni-icons", { attrs: { type: "arrowright", color: A.moreColor, size: "14" } }),
											],
											1,
									  )
									: A._e(),
							],
							1,
					  )
					: A._e();
			},
			i = [];
	},
	593: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(288),
			t = E.n(e);
		for (var i in e)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(i);
		B.default = t.a;
	},
	594: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(291),
			t = E.n(e);
		for (var i in e)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(i);
		B.default = t.a;
	},
	6: function (A, B, E) {
		(Vue.prototype.__$appStyle__ = {}), Vue.prototype.__merge_style && Vue.prototype.__merge_style(E(7).default, Vue.prototype.__$appStyle__);
	},
	7: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(1),
			t = E.n(e);
		for (var i in e)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(i);
		B.default = t.a;
	},
	701: function (A, B, E) {
		"use strict";
		E.r(B);
		E(6);
		var e = E(98);
		"undefined" == typeof Promise ||
			Promise.prototype.finally ||
			(Promise.prototype.finally = function (A) {
				var B = this.constructor;
				return this.then(
					function (E) {
						return B.resolve(A()).then(function () {
							return E;
						});
					},
					function (E) {
						return B.resolve(A()).then(function () {
							throw E;
						});
					},
				);
			}),
			(e.default.mpType = "page"),
			(e.default.route = "pages/extUI/notice-bar/notice-bar"),
			(e.default.el = "#root"),
			new Vue(e.default);
	},
	702: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(511),
			t = E(286);
		for (var i in t)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return t[A];
					});
				})(i);
		var g = E(0);
		var Q = Object(g.a)(t.default, e.b, e.c, !1, null, "130426e9", "9376c062", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(593).default, this.options.style) : Object.assign(this.options.style, E(593).default);
		}).call(Q),
			(B.default = Q.exports);
	},
	8: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(4),
			t = E.n(e);
		for (var i in e)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(i);
		B.default = t.a;
	},
	9: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(10),
			t = E.n(e);
		for (var i in e)
			"default" !== i &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(i);
		B.default = t.a;
	},
	98: function (A, B, E) {
		"use strict";
		var e = E(483),
			t = E(289),
			i = E(0);
		var g = Object(i.a)(t.default, e.b, e.c, !1, null, null, "5ab23a4c", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(594).default, this.options.style) : Object.assign(this.options.style, E(594).default);
		}).call(g),
			(B.default = g.exports);
	},
});
