!(function (A) {
	var B = {};
	function E(e) {
		if (B[e]) return B[e].exports;
		var t = (B[e] = { i: e, l: !1, exports: {} });
		return A[e].call(t.exports, t, t.exports, E), (t.l = !0), t.exports;
	}
	(E.m = A),
		(E.c = B),
		(E.d = function (A, B, e) {
			E.o(A, B) || Object.defineProperty(A, B, { enumerable: !0, get: e });
		}),
		(E.r = function (A) {
			"undefined" != typeof Symbol && Symbol.toStringTag && Object.defineProperty(A, Symbol.toStringTag, { value: "Module" }),
				Object.defineProperty(A, "__esModule", { value: !0 });
		}),
		(E.t = function (A, B) {
			if ((1 & B && (A = E(A)), 8 & B)) return A;
			if (4 & B && "object" == typeof A && A && A.__esModule) return A;
			var e = Object.create(null);
			if ((E.r(e), Object.defineProperty(e, "default", { enumerable: !0, value: A }), 2 & B && "string" != typeof A))
				for (var t in A)
					E.d(
						e,
						t,
						function (B) {
							return A[B];
						}.bind(null, t),
					);
			return e;
		}),
		(E.n = function (A) {
			var B =
				A && A.__esModule
					? function () {
							return A.default;
					  }
					: function () {
							return A;
					  };
			return E.d(B, "a", B), B;
		}),
		(E.o = function (A, B) {
			return Object.prototype.hasOwnProperty.call(A, B);
		}),
		(E.p = ""),
		E((E.s = 714));
})({
	0: function (A, B, E) {
		"use strict";
		function e(A, B, E, e, t, c, Q, g, i, n) {
			var o,
				l = "function" == typeof A ? A.options : A;
			if (i) {
				l.components || (l.components = {});
				var w = Object.prototype.hasOwnProperty;
				for (var s in i) w.call(i, s) && !w.call(l.components, s) && (l.components[s] = i[s]);
			}
			if (
				(n &&
					((n.beforeCreate || (n.beforeCreate = [])).unshift(function () {
						this[n.__module] = this;
					}),
					(l.mixins || (l.mixins = [])).push(n)),
				B && ((l.render = B), (l.staticRenderFns = E), (l._compiled = !0)),
				e && (l.functional = !0),
				c && (l._scopeId = "data-v-" + c),
				Q
					? ((o = function (A) {
							(A = A || (this.$vnode && this.$vnode.ssrContext) || (this.parent && this.parent.$vnode && this.parent.$vnode.ssrContext)) ||
								"undefined" == typeof __VUE_SSR_CONTEXT__ ||
								(A = __VUE_SSR_CONTEXT__),
								t && t.call(this, A),
								A && A._registeredComponents && A._registeredComponents.add(Q);
					  }),
					  (l._ssrRegister = o))
					: t &&
					  (o = g
							? function () {
									t.call(this, this.$root.$options.shadowRoot);
							  }
							: t),
				o)
			)
				if (l.functional) {
					l._injectStyles = o;
					var a = l.render;
					l.render = function (A, B) {
						return o.call(B), a(A, B);
					};
				} else {
					var F = l.beforeCreate;
					l.beforeCreate = F ? [].concat(F, o) : [o];
				}
			return { exports: A, options: l };
		}
		E.d(B, "a", function () {
			return e;
		});
	},
	1: function (A, B) {
		A.exports = { "@VERSION": 2 };
	},
	10: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e,
			t = (e = E(17)) && e.__esModule ? e : { default: e };
		weex
			.requireModule("dom")
			.addRule("fontFace", {
				fontFamily: "uniicons",
				src: "url('data:font/truetype;charset=utf-8;base64,')",
			});
		var c = {
			name: "UniIcons",
			props: {
				type: { type: String, default: "" },
				color: { type: String, default: "#333333" },
				size: { type: [Number, String], default: 16 },
				customIcons: { type: String, default: "" },
			},
			data: function () {
				return { icons: t.default };
			},
			methods: {
				_onClick: function () {
					this.$emit("click");
				},
			},
		};
		B.default = c;
	},
	104: function (A, B, E) {
		"use strict";
		var e = E(464),
			t = E(331),
			c = E(0);
		var Q = Object(c.a)(t.default, e.b, e.c, !1, null, null, "08dd767a", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(608).default, this.options.style) : Object.assign(this.options.style, E(608).default);
		}).call(Q),
			(B.default = Q.exports);
	},
	11: function (A, B) {
		A.exports = {
			".uni-icons": { "": { fontFamily: ["uniicons", 0, 0, 0], textDecoration: ["none", 0, 0, 0], textAlign: ["center", 0, 0, 0] } },
			"@VERSION": 2,
		};
	},
	12: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return e;
		}),
			E.d(B, "c", function () {
				return t;
			}),
			E.d(B, "a", function () {});
		var e = function () {
				var A = this,
					B = A.$createElement;
				return (A._self._c || B)(
					"u-text",
					{
						staticClass: ["uni-icons"],
						class: [A.customIcons, A.customIcons ? A.type : ""],
						style: { color: A.color, "font-size": A.size + "px" },
						appendAsTree: !0,
						attrs: { append: "tree" },
						on: { click: A._onClick },
					},
					[A._v(A._s(A.icons[A.type]))],
				);
			},
			t = [];
	},
	13: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(5),
			t = E(2);
		for (var c in t)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return t[A];
					});
				})(c);
		var Q = E(0);
		var g = Object(Q.a)(t.default, e.b, e.c, !1, null, "168bca6c", "39eb9647", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(8).default, this.options.style) : Object.assign(this.options.style, E(8).default);
		}).call(g),
			(B.default = g.exports);
	},
	15: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(11),
			t = E.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = t.a;
	},
	16: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(12),
			t = E(9);
		for (var c in t)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return t[A];
					});
				})(c);
		var Q = E(0);
		var g = Object(Q.a)(t.default, e.b, e.c, !1, null, "041e7dc0", "67d6d51e", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(15).default, this.options.style) : Object.assign(this.options.style, E(15).default);
		}).call(g),
			(B.default = g.exports);
	},
	17: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		B.default = {
			pulldown: "\ue588",
			refreshempty: "\ue461",
			back: "\ue471",
			forward: "\ue470",
			more: "\ue507",
			"more-filled": "\ue537",
			scan: "\ue612",
			qq: "\ue264",
			weibo: "\ue260",
			weixin: "\ue261",
			pengyouquan: "\ue262",
			loop: "\ue565",
			refresh: "\ue407",
			"refresh-filled": "\ue437",
			arrowthindown: "\ue585",
			arrowthinleft: "\ue586",
			arrowthinright: "\ue587",
			arrowthinup: "\ue584",
			"undo-filled": "\ue7d6",
			undo: "\ue406",
			redo: "\ue405",
			"redo-filled": "\ue7d9",
			bars: "\ue563",
			chatboxes: "\ue203",
			camera: "\ue301",
			"chatboxes-filled": "\ue233",
			"camera-filled": "\ue7ef",
			"cart-filled": "\ue7f4",
			cart: "\ue7f5",
			"checkbox-filled": "\ue442",
			checkbox: "\ue7fa",
			arrowleft: "\ue582",
			arrowdown: "\ue581",
			arrowright: "\ue583",
			"smallcircle-filled": "\ue801",
			arrowup: "\ue580",
			circle: "\ue411",
			"eye-filled": "\ue568",
			"eye-slash-filled": "\ue822",
			"eye-slash": "\ue823",
			eye: "\ue824",
			"flag-filled": "\ue825",
			flag: "\ue508",
			"gear-filled": "\ue532",
			reload: "\ue462",
			gear: "\ue502",
			"hand-thumbsdown-filled": "\ue83b",
			"hand-thumbsdown": "\ue83c",
			"hand-thumbsup-filled": "\ue83d",
			"heart-filled": "\ue83e",
			"hand-thumbsup": "\ue83f",
			heart: "\ue840",
			home: "\ue500",
			info: "\ue504",
			"home-filled": "\ue530",
			"info-filled": "\ue534",
			"circle-filled": "\ue441",
			"chat-filled": "\ue847",
			chat: "\ue263",
			"mail-open-filled": "\ue84d",
			"email-filled": "\ue231",
			"mail-open": "\ue84e",
			email: "\ue201",
			checkmarkempty: "\ue472",
			list: "\ue562",
			"locked-filled": "\ue856",
			locked: "\ue506",
			"map-filled": "\ue85c",
			"map-pin": "\ue85e",
			"map-pin-ellipse": "\ue864",
			map: "\ue364",
			"minus-filled": "\ue440",
			"mic-filled": "\ue332",
			minus: "\ue410",
			micoff: "\ue360",
			mic: "\ue302",
			clear: "\ue434",
			smallcircle: "\ue868",
			close: "\ue404",
			closeempty: "\ue460",
			paperclip: "\ue567",
			paperplane: "\ue503",
			"paperplane-filled": "\ue86e",
			"person-filled": "\ue131",
			"contact-filled": "\ue130",
			person: "\ue101",
			contact: "\ue100",
			"images-filled": "\ue87a",
			phone: "\ue200",
			images: "\ue87b",
			image: "\ue363",
			"image-filled": "\ue877",
			"location-filled": "\ue333",
			location: "\ue303",
			"plus-filled": "\ue439",
			plus: "\ue409",
			plusempty: "\ue468",
			"help-filled": "\ue535",
			help: "\ue505",
			"navigate-filled": "\ue884",
			navigate: "\ue501",
			"mic-slash-filled": "\ue892",
			search: "\ue466",
			settings: "\ue560",
			sound: "\ue590",
			"sound-filled": "\ue8a1",
			"spinner-cycle": "\ue465",
			"download-filled": "\ue8a4",
			"personadd-filled": "\ue132",
			"videocam-filled": "\ue8af",
			personadd: "\ue102",
			upload: "\ue402",
			"upload-filled": "\ue8b1",
			starhalf: "\ue463",
			"star-filled": "\ue438",
			star: "\ue408",
			trash: "\ue401",
			"phone-filled": "\ue230",
			compose: "\ue400",
			videocam: "\ue300",
			"trash-filled": "\ue8dc",
			download: "\ue403",
			"chatbubble-filled": "\ue232",
			chatbubble: "\ue202",
			"cloud-download": "\ue8e4",
			"cloud-upload-filled": "\ue8e5",
			"cloud-upload": "\ue8e6",
			"cloud-download-filled": "\ue8e9",
			headphones: "\ue8bf",
			shop: "\ue609",
		};
	},
	2: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(3),
			t = E.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = t.a;
	},
	3: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e = {
			name: "UniSection",
			props: { type: { type: String, default: "" }, title: { type: String, default: "" }, subTitle: { type: String, default: "" } },
			data: function () {
				return {};
			},
			watch: {
				title: function (A) {
					uni.report && "" !== A && uni.report("title", A);
				},
			},
			methods: {
				onClick: function () {
					this.$emit("click");
				},
			},
		};
		B.default = e;
	},
	325: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(326),
			t = E.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = t.a;
	},
	326: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e = {
			name: "UniCollapse",
			props: { accordion: { type: [Boolean, String], default: !1 } },
			data: function () {
				return {};
			},
			provide: function () {
				return { collapse: this };
			},
			created: function () {
				this.childrens = [];
			},
			methods: {
				onChange: function () {
					var A = [];
					this.childrens.forEach(function (B, E) {
						B.isOpen && A.push(B.nameSync);
					}),
						this.$emit("change", A);
				},
			},
		};
		B.default = e;
	},
	327: function (A, B) {
		A.exports = { ".uni-collapse": { "": { flex: [1, 0, 0, 0], flexDirection: ["column", 0, 0, 0], backgroundColor: ["#ffffff", 0, 0, 0] } }, "@VERSION": 2 };
	},
	328: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(329),
			t = E.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = t.a;
	},
	329: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var e = {
			name: "UniCollapseItem",
			props: {
				title: { type: String, default: "" },
				name: { type: [Number, String], default: 0 },
				disabled: { type: Boolean, default: !1 },
				showAnimation: { type: Boolean, default: !1 },
				open: { type: Boolean, default: !1 },
				thumb: { type: String, default: "" },
			},
			data: function () {
				return { isOpen: !1 };
			},
			watch: {
				open: function (A) {
					this.isOpen = A;
				},
			},
			inject: ["collapse"],
			created: function () {
				((this.isOpen = this.open),
				(this.nameSync = this.name ? this.name : this.collapse.childrens.length),
				this.collapse.childrens.push(this),
				"true" === String(this.collapse.accordion)) &&
					this.isOpen &&
					this.collapse.childrens[this.collapse.childrens.length - 2] &&
					(this.collapse.childrens[this.collapse.childrens.length - 2].isOpen = !1);
			},
			methods: {
				onClick: function () {
					var A = this;
					this.disabled ||
						("true" === String(this.collapse.accordion) &&
							this.collapse.childrens.forEach(function (B) {
								B !== A && (B.isOpen = !1);
							}),
						(this.isOpen = !this.isOpen),
						this.collapse.onChange && this.collapse.onChange(),
						this.$forceUpdate());
				},
			},
		};
		B.default = e;
	},
	330: function (A, B) {
		A.exports = {
			".uni-collapse-cell": {
				"": {
					flexDirection: ["column", 0, 0, 16],
					borderColor: ["#e5e5e5", 0, 0, 16],
					borderBottomWidth: ["1", 0, 0, 16],
					borderBottomStyle: ["solid", 0, 0, 16],
				},
			},
			".uni-collapse-cell--hover": { "": { backgroundColor: ["#f1f1f1", 0, 0, 17] } },
			".uni-collapse-cell--open": { "": { backgroundColor: ["#f1f1f1", 0, 0, 18] } },
			".uni-collapse-cell--disabled": { "": { backgroundColor: ["#f1f1f1", 0, 0, 19] } },
			".uni-collapse-cell--hide": { "": { height: ["48", 0, 0, 20] } },
			".uni-collapse-cell--animation": {
				"": { transitionProperty: ["transform", 0, 0, 21], transitionDuration: [300, 0, 0, 21], transitionTimingFunction: ["ease", 0, 0, 21] },
			},
			".uni-collapse-cell__title": {
				"": {
					paddingTop: ["12", 0, 0, 22],
					paddingRight: ["12", 0, 0, 22],
					paddingBottom: ["12", 0, 0, 22],
					paddingLeft: ["12", 0, 0, 22],
					position: ["relative", 0, 0, 22],
					height: ["48", 0, 0, 22],
					lineHeight: ["24", 0, 0, 22],
					flexDirection: ["row", 0, 0, 22],
					justifyContent: ["space-between", 0, 0, 22],
					alignItems: ["center", 0, 0, 22],
					"backgroundColor:active": ["#f1f1f1", 0, 0, 23],
				},
			},
			".uni-collapse-cell__title-img": { "": { height: ["26", 0, 0, 24], width: ["26", 0, 0, 24], marginRight: ["10", 0, 0, 24] } },
			".uni-collapse-cell__title-arrow": {
				"": { width: ["20", 0, 0, 25], height: ["20", 0, 0, 25], transform: ["rotate(0deg)", 0, 0, 25], transformOrigin: ["center center", 0, 0, 25] },
			},
			".uni-collapse-cell__title-arrow-active": { "": { transform: ["rotate(180deg)", 0, 0, 26] } },
			".uni-collapse-cell__title-text": {
				"": { flex: [1, 0, 0, 27], fontSize: ["14", 0, 0, 27], lines: [1, 0, 0, 27], overflow: ["hidden", 0, 0, 27], textOverflow: ["ellipsis", 0, 0, 27] },
			},
			".uni-collapse-cell__content": { "": { overflow: ["hidden", 0, 0, 28] } },
			".uni-collapse-cell__wrapper": { "": { flexDirection: ["column", 0, 0, 29] } },
			".uni-collapse-cell__content--hide": { "": { height: ["0", 0, 0, 30], lineHeight: ["0", 0, 0, 30] } },
			"@VERSION": 2,
		};
	},
	331: function (A, B, E) {
		"use strict";
		var e = E(332),
			t = E.n(e);
		B.default = t.a;
	},
	332: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		B.default = {
			components: {},
			data: function () {
				return {
					list: [
						{
							name: "\u57fa\u7840\u7528\u6cd5",
							data: [
								{
									type: !1,
									subName: "\u9ed8\u8ba4\u5f00\u542f",
									open: !0,
									content: "\u6298\u53e0\u5185\u5bb9\u4e3b\u4f53\uff0c\u53ef\u81ea\u5b9a\u4e49\u5185\u5bb9\u53ca\u6837\u5f0f",
								},
								{
									type: !1,
									subName: "\u6298\u53e0\u5185\u5bb9",
									content:
										"\u6298\u53e0\u5185\u5bb9\u4e3b\u4f53\uff0c\u8fd9\u662f\u4e00\u6bb5\u6bd4\u8f83\u957f\u5185\u5bb9\u3002\u9ed8\u8ba4\u6298\u53e0\u4e3b\u8981\u5185\u5bb9\uff0c\u53ea\u663e\u793a\u5f53\u524d\u9879\u6807\u9898\u3002\u70b9\u51fb\u6807\u9898\u5c55\u5f00\uff0c\u624d\u80fd\u770b\u5230\u8fd9\u6bb5\u95ee\u9898\u3002\u518d\u6b21\u70b9\u51fb\u6807\u9898\uff0c\u6298\u53e0\u5185\u5bb9\u3002",
								},
								{
									type: !1,
									subName: "\u7981\u7528\u72b6\u6001",
									disabled: !0,
									content:
										"\u6298\u53e0\u5185\u5bb9\u4e3b\u4f53\uff0c\u8fd9\u662f\u4e00\u6bb5\u6bd4\u8f83\u957f\u5185\u5bb9\u3002\u9ed8\u8ba4\u6298\u53e0\u4e3b\u8981\u5185\u5bb9\uff0c\u53ea\u663e\u793a\u5f53\u524d\u9879\u6807\u9898\u3002\u70b9\u51fb\u6807\u9898\u5c55\u5f00\uff0c\u624d\u80fd\u770b\u5230\u8fd9\u6bb5\u95ee\u9898\u3002\u518d\u6b21\u70b9\u51fb\u6807\u9898\uff0c\u6298\u53e0\u5185\u5bb9\u3002",
								},
							],
						},
						{
							name: "\u6dfb\u52a0\u52a8\u753b\u6548\u679c",
							data: [
								{
									type: !0,
									subName: "\u52a8\u753b\u6548\u679c",
									showAnimation: !0,
									content:
										"\u6298\u53e0\u5185\u5bb9\u4e3b\u4f53\uff0c\u8fd9\u662f\u4e00\u6bb5\u6bd4\u8f83\u957f\u5185\u5bb9\u3002\u9ed8\u8ba4\u6298\u53e0\u4e3b\u8981\u5185\u5bb9\uff0c\u53ea\u663e\u793a\u5f53\u524d\u9879\u6807\u9898\u3002\u70b9\u51fb\u6807\u9898\u5c55\u5f00\uff0c\u624d\u80fd\u770b\u5230\u8fd9\u6bb5\u95ee\u9898\u3002\u518d\u6b21\u70b9\u51fb\u6807\u9898\uff0c\u6298\u53e0\u5185\u5bb9\u3002",
								},
							],
						},
					],
					accordion: [
						{ id: 0, title: "\u6807\u9898\u6587\u5b57", content: "\u624b\u98ce\u7434\u6548\u679c", animation: !0 },
						{ id: 1, title: "\u6807\u9898\u6587\u5b57", content: "\u624b\u98ce\u7434\u6548\u679c", animation: !0 },
						{ id: 2, title: "\u6807\u9898\u6587\u5b57", content: "\u624b\u98ce\u7434\u6548\u679c", animation: !0 },
					],
					extraIcon: { color: "#4cd964", size: "26", type: "image" },
					id: 2,
				};
			},
			methods: { change: function (A) {} },
		};
	},
	333: function (A, B) {
		A.exports = {
			".example": { "": { paddingTop: [0, 0, 0, 2], paddingRight: ["15", 0, 0, 2], paddingBottom: [0, 0, 0, 2], paddingLeft: ["15", 0, 0, 2] } },
			".example-info": {
				"": {
					paddingTop: ["15", 0, 0, 3],
					paddingRight: ["15", 0, 0, 3],
					paddingBottom: ["15", 0, 0, 3],
					paddingLeft: ["15", 0, 0, 3],
					color: ["#3b4144", 0, 0, 3],
					backgroundColor: ["#ffffff", 0, 0, 3],
					fontSize: ["14", 0, 0, 3],
					lineHeight: ["20", 0, 0, 3],
				},
			},
			".example-info-text": { "": { fontSize: ["14", 0, 0, 4], lineHeight: ["20", 0, 0, 4], color: ["#3b4144", 0, 0, 4] } },
			".example-body": {
				"": {
					flexDirection: ["column", 0, 0, 9],
					paddingTop: ["15", 0, 0, 5],
					paddingRight: ["15", 0, 0, 5],
					paddingBottom: ["15", 0, 0, 5],
					paddingLeft: ["15", 0, 0, 5],
					backgroundColor: ["#ffffff", 0, 0, 5],
					flex: [1, 0, 0, 9],
				},
			},
			".word-btn-white": { "": { fontSize: ["18", 0, 0, 6], color: ["#FFFFFF", 0, 0, 6] } },
			".word-btn": {
				"": {
					flexDirection: ["row", 0, 0, 7],
					alignItems: ["center", 0, 0, 7],
					justifyContent: ["center", 0, 0, 7],
					borderRadius: ["6", 0, 0, 7],
					height: ["48", 0, 0, 7],
					marginTop: ["15", 0, 0, 7],
					marginRight: ["15", 0, 0, 7],
					marginBottom: ["15", 0, 0, 7],
					marginLeft: ["15", 0, 0, 7],
					backgroundColor: ["#007AFF", 0, 0, 7],
				},
			},
			".word-btn--hover": { "": { backgroundColor: ["#4ca2ff", 0, 0, 8] } },
			".content": {
				"": {
					paddingTop: ["15", 0, 0, 10],
					paddingRight: ["15", 0, 0, 10],
					paddingBottom: ["15", 0, 0, 10],
					paddingLeft: ["15", 0, 0, 10],
					fontSize: ["14", 0, 0, 10],
					lineHeight: ["20", 0, 0, 10],
					backgroundColor: ["#f9f9f9", 0, 0, 10],
					color: ["#666666", 0, 0, 10],
				},
			},
			".button": { "": { fontSize: ["26rpx", 0, 0, 11], lineHeight: ["90rpx", 0, 0, 11] } },
			"@VERSION": 2,
		};
	},
	4: function (A, B) {
		A.exports = {
			".uni-section": {
				"": {
					position: ["relative", 0, 0, 0],
					marginTop: ["10", 0, 0, 0],
					flexDirection: ["row", 0, 0, 0],
					alignItems: ["center", 0, 0, 0],
					paddingTop: [0, 0, 0, 0],
					paddingRight: ["10", 0, 0, 0],
					paddingBottom: [0, 0, 0, 0],
					paddingLeft: ["10", 0, 0, 0],
					height: ["50", 0, 0, 0],
					backgroundColor: ["#f8f8f8", 0, 0, 0],
					fontWeight: ["normal", 0, 0, 0],
				},
			},
			".uni-section__head": {
				"": { flexDirection: ["row", 0, 0, 1], justifyContent: ["center", 0, 0, 1], alignItems: ["center", 0, 0, 1], marginRight: ["10", 0, 0, 1] },
			},
			".line": { "": { height: ["15", 0, 0, 2], backgroundColor: ["#c0c0c0", 0, 0, 2], borderRadius: ["5", 0, 0, 2], width: ["3", 0, 0, 2] } },
			".circle": {
				"": {
					width: ["8", 0, 0, 3],
					height: ["8", 0, 0, 3],
					borderTopRightRadius: ["50", 0, 0, 3],
					borderTopLeftRadius: ["50", 0, 0, 3],
					borderBottomLeftRadius: ["50", 0, 0, 3],
					borderBottomRightRadius: ["50", 0, 0, 3],
					backgroundColor: ["#c0c0c0", 0, 0, 3],
				},
			},
			".uni-section__content": { "": { flexDirection: ["column", 0, 0, 4], flex: [1, 0, 0, 4], color: ["#333333", 0, 0, 4] } },
			".uni-section__content-title": { "": { fontSize: ["14", 0, 0, 5], color: ["#333333", 0, 0, 5] } },
			".distraction": { "": { flexDirection: ["row", 0, 0, 6], alignItems: ["center", 0, 0, 6] } },
			".uni-section__content-sub": { "": { fontSize: ["12", 0, 0, 7], color: ["#999999", 0, 0, 7] } },
			"@VERSION": 2,
		};
	},
	464: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return t;
		}),
			E.d(B, "c", function () {
				return c;
			}),
			E.d(B, "a", function () {
				return e;
			});
		var e = { uniSection: E(13).default, uniCollapse: E(715).default, uniCollapseItem: E(716).default },
			t = function () {
				var A = this,
					B = A.$createElement,
					E = A._self._c || B;
				return E("scroll-view", { staticStyle: { flexDirection: "column" }, attrs: { scrollY: !0, showScrollbar: !0, enableBackToTop: !0, bubble: "true" } }, [
					E(
						"view",
						[
							E("u-text", { staticClass: ["example-info"], appendAsTree: !0, attrs: { append: "tree" } }, [
								A._v(
									"\u6298\u53e0\u9762\u677f\u7528\u6765\u6298\u53e0/\u663e\u793a\u8fc7\u957f\u7684\u5185\u5bb9\u6216\u8005\u662f\u5217\u8868\u3002\u901a\u5e38\u662f\u5728\u591a\u5185\u5bb9\u5206\u7c7b\u9879\u4f7f\u7528\uff0c\u6298\u53e0\u4e0d\u91cd\u8981\u7684\u5185\u5bb9\uff0c\u663e\u793a\u91cd\u8981\u5185\u5bb9\u3002\u70b9\u51fb\u53ef\u4ee5\u5c55\u5f00\u6298\u53e0\u90e8\u5206\u3002",
								),
							]),
							A._l(A.list, function (B, e) {
								return E(
									"view",
									{ key: e },
									[
										E("uni-section", { attrs: { title: B.name, type: "line" } }),
										E(
											"uni-collapse",
											{ ref: "add", refInFor: !0, staticClass: ["warp"], on: { change: A.change } },
											A._l(B.data, function (B, e) {
												return E(
													"uni-collapse-item",
													{ key: e, attrs: { open: B.open, showAnimation: B.showAnimation, disabled: B.disabled, title: B.subName } },
													[E("u-text", { staticClass: ["content"], appendAsTree: !0, attrs: { append: "tree" } }, [A._v(A._s(B.content))])],
												);
											}),
											1,
										),
									],
									1,
								);
							}),
							E("uni-section", {
								attrs: { title: "\u624b\u98ce\u7434\u6548\u679c\uff08\u5c55\u5f00\u4e00\u4e2a\u4f1a\u6536\u8d77\u5176\u4ed6\uff09", type: "line" },
							}),
							E(
								"uni-collapse",
								{ attrs: { accordion: !0 } },
								A._l(A.accordion, function (B) {
									return E("uni-collapse-item", { key: B.id, attrs: { title: B.title, showAnimation: B.animation } }, [
										E("u-text", { staticClass: ["content"], appendAsTree: !0, attrs: { append: "tree" } }, [A._v(A._s(B.content))]),
									]);
								}),
								1,
							),
							E("uni-section", { attrs: { title: "\u914d\u7f6e\u56fe\u6807", type: "line" } }),
							E(
								"uni-collapse",
								[
									E(
										"uni-collapse-item",
										{
											attrs: {
												title: "\u6807\u9898\u6587\u5b57",
												thumb: "https://vkceyugu.cdn.bspapp.com/VKCEYUGU-dc-site/460d46d0-4fcc-11eb-8ff1-d5dcf8779628.png",
											},
										},
										[
											E("u-text", { staticClass: ["content"], appendAsTree: !0, attrs: { append: "tree" } }, [
												A._v("\u6298\u53e0\u5185\u5bb9\u4e3b\u4f53\uff0c\u53ef\u81ea\u5b9a\u4e49\u5185\u5bb9\u53ca\u6837\u5f0f"),
											]),
										],
									),
									E(
										"uni-collapse-item",
										{
											attrs: {
												title: "\u6807\u9898\u6587\u5b57",
												thumb: "https://vkceyugu.cdn.bspapp.com/VKCEYUGU-dc-site/460d46d0-4fcc-11eb-8ff1-d5dcf8779628.png",
											},
										},
										[
											E("u-text", { staticClass: ["content"], appendAsTree: !0, attrs: { append: "tree" } }, [
												A._v("\u6298\u53e0\u5185\u5bb9\u4e3b\u4f53\uff0c\u53ef\u81ea\u5b9a\u4e49\u5185\u5bb9\u53ca\u6837\u5f0f"),
											]),
										],
									),
								],
								1,
							),
						],
						2,
					),
				]);
			},
			c = [];
	},
	5: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return e;
		}),
			E.d(B, "c", function () {
				return t;
			}),
			E.d(B, "a", function () {});
		var e = function () {
				var A = this,
					B = A.$createElement,
					E = A._self._c || B;
				return E(
					"view",
					{ staticClass: ["uni-section"], attrs: { nvue: !0 } },
					[
						A.type ? E("view", { staticClass: ["uni-section__head"] }, [E("view", { staticClass: ["uni-section__head-tag"], class: A.type })]) : A._e(),
						E("view", { staticClass: ["uni-section__content"] }, [
							E("u-text", { staticClass: ["uni-section__content-title"], class: { distraction: !A.subTitle }, appendAsTree: !0, attrs: { append: "tree" } }, [
								A._v(A._s(A.title)),
							]),
							A.subTitle
								? E("u-text", { staticClass: ["uni-section__content-sub"], appendAsTree: !0, attrs: { append: "tree" } }, [A._v(A._s(A.subTitle))])
								: A._e(),
						]),
						A._t("default"),
					],
					2,
				);
			},
			t = [];
	},
	500: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return e;
		}),
			E.d(B, "c", function () {
				return t;
			}),
			E.d(B, "a", function () {});
		var e = function () {
				var A = this.$createElement;
				return (this._self._c || A)("view", { staticClass: ["uni-collapse"] }, [this._t("default")], 2);
			},
			t = [];
	},
	501: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return t;
		}),
			E.d(B, "c", function () {
				return c;
			}),
			E.d(B, "a", function () {
				return e;
			});
		var e = { uniIcons: E(16).default },
			t = function () {
				var A = this,
					B = A.$createElement,
					E = A._self._c || B;
				return E(
					"view",
					{
						staticClass: ["uni-collapse-cell"],
						class: {
							"uni-collapse-cell--disabled": A.disabled,
							"uni-collapse-cell--notdisabled": !A.disabled,
							"uni-collapse-cell--open": A.isOpen,
							"uni-collapse-cell--hide": !A.isOpen,
						},
					},
					[
						E(
							"view",
							{ staticClass: ["uni-collapse-cell__title"], class: { "uni-collapse-cell--disabled": A.disabled }, on: { click: A.onClick } },
							[
								A.thumb ? E("u-image", { staticClass: ["uni-collapse-cell__title-img"], attrs: { src: A.thumb } }) : A._e(),
								E("u-text", { staticClass: ["uni-collapse-cell__title-text"], appendAsTree: !0, attrs: { append: "tree" } }, [A._v(A._s(A.title))]),
								E("uni-icons", {
									staticClass: ["uni-collapse-cell__title-arrow"],
									class: { "uni-collapse-cell__title-arrow-active": A.isOpen, "uni-collapse-cell--animation": !0 === A.showAnimation },
									attrs: { color: "#bbb", size: "20", type: "arrowdown" },
								}),
							],
							1,
						),
						E("view", { staticClass: ["uni-collapse-cell__content"], class: { "uni-collapse-cell__content--hide": !A.isOpen } }, [
							E(
								"view",
								{
									staticClass: ["uni-collapse-cell__wrapper"],
									class: { "uni-collapse-cell--animation": !0 === A.showAnimation },
									style: { transform: A.isOpen ? "translateY(0)" : "translateY(-50%)", "-webkit-transform": A.isOpen ? "translateY(0)" : "translateY(-50%)" },
								},
								[A._t("default")],
								2,
							),
						]),
					],
				);
			},
			c = [];
	},
	6: function (A, B, E) {
		(Vue.prototype.__$appStyle__ = {}), Vue.prototype.__merge_style && Vue.prototype.__merge_style(E(7).default, Vue.prototype.__$appStyle__);
	},
	606: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(327),
			t = E.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = t.a;
	},
	607: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(330),
			t = E.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = t.a;
	},
	608: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(333),
			t = E.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = t.a;
	},
	7: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(1),
			t = E.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = t.a;
	},
	714: function (A, B, E) {
		"use strict";
		E.r(B);
		E(6);
		var e = E(104);
		"undefined" == typeof Promise ||
			Promise.prototype.finally ||
			(Promise.prototype.finally = function (A) {
				var B = this.constructor;
				return this.then(
					function (E) {
						return B.resolve(A()).then(function () {
							return E;
						});
					},
					function (E) {
						return B.resolve(A()).then(function () {
							throw E;
						});
					},
				);
			}),
			(e.default.mpType = "page"),
			(e.default.route = "pages/extUI/collapse/collapse"),
			(e.default.el = "#root"),
			new Vue(e.default);
	},
	715: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(500),
			t = E(325);
		for (var c in t)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return t[A];
					});
				})(c);
		var Q = E(0);
		var g = Object(Q.a)(t.default, e.b, e.c, !1, null, "506c0e2e", "57bbbc22", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(606).default, this.options.style) : Object.assign(this.options.style, E(606).default);
		}).call(g),
			(B.default = g.exports);
	},
	716: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(501),
			t = E(328);
		for (var c in t)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return t[A];
					});
				})(c);
		var Q = E(0);
		var g = Object(Q.a)(t.default, e.b, e.c, !1, null, "000bbe84", "272f013a", !1, e.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(607).default, this.options.style) : Object.assign(this.options.style, E(607).default);
		}).call(g),
			(B.default = g.exports);
	},
	8: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(4),
			t = E.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = t.a;
	},
	9: function (A, B, E) {
		"use strict";
		E.r(B);
		var e = E(10),
			t = E.n(e);
		for (var c in e)
			"default" !== c &&
				(function (A) {
					E.d(B, A, function () {
						return e[A];
					});
				})(c);
		B.default = t.a;
	},
});
