!(function (A) {
	var B = {};
	function E(g) {
		if (B[g]) return B[g].exports;
		var Q = (B[g] = { i: g, l: !1, exports: {} });
		return A[g].call(Q.exports, Q, Q.exports, E), (Q.l = !0), Q.exports;
	}
	(E.m = A),
		(E.c = B),
		(E.d = function (A, B, g) {
			E.o(A, B) || Object.defineProperty(A, B, { enumerable: !0, get: g });
		}),
		(E.r = function (A) {
			"undefined" != typeof Symbol && Symbol.toStringTag && Object.defineProperty(A, Symbol.toStringTag, { value: "Module" }),
				Object.defineProperty(A, "__esModule", { value: !0 });
		}),
		(E.t = function (A, B) {
			if ((1 & B && (A = E(A)), 8 & B)) return A;
			if (4 & B && "object" == typeof A && A && A.__esModule) return A;
			var g = Object.create(null);
			if ((E.r(g), Object.defineProperty(g, "default", { enumerable: !0, value: A }), 2 & B && "string" != typeof A))
				for (var Q in A)
					E.d(
						g,
						Q,
						function (B) {
							return A[B];
						}.bind(null, Q),
					);
			return g;
		}),
		(E.n = function (A) {
			var B =
				A && A.__esModule
					? function () {
							return A.default;
					  }
					: function () {
							return A;
					  };
			return E.d(B, "a", B), B;
		}),
		(E.o = function (A, B) {
			return Object.prototype.hasOwnProperty.call(A, B);
		}),
		(E.p = ""),
		E((E.s = 678));
})({
	0: function (A, B, E) {
		"use strict";
		function g(A, B, E, g, Q, t, c, e, o, i) {
			var n,
				w = "function" == typeof A ? A.options : A;
			if (o) {
				w.components || (w.components = {});
				var C = Object.prototype.hasOwnProperty;
				for (var F in o) C.call(o, F) && !C.call(w.components, F) && (w.components[F] = o[F]);
			}
			if (
				(i &&
					((i.beforeCreate || (i.beforeCreate = [])).unshift(function () {
						this[i.__module] = this;
					}),
					(w.mixins || (w.mixins = [])).push(i)),
				B && ((w.render = B), (w.staticRenderFns = E), (w._compiled = !0)),
				g && (w.functional = !0),
				t && (w._scopeId = "data-v-" + t),
				c
					? ((n = function (A) {
							(A = A || (this.$vnode && this.$vnode.ssrContext) || (this.parent && this.parent.$vnode && this.parent.$vnode.ssrContext)) ||
								"undefined" == typeof __VUE_SSR_CONTEXT__ ||
								(A = __VUE_SSR_CONTEXT__),
								Q && Q.call(this, A),
								A && A._registeredComponents && A._registeredComponents.add(c);
					  }),
					  (w._ssrRegister = n))
					: Q &&
					  (n = e
							? function () {
									Q.call(this, this.$root.$options.shadowRoot);
							  }
							: Q),
				n)
			)
				if (w.functional) {
					w._injectStyles = n;
					var I = w.render;
					w.render = function (A, B) {
						return n.call(B), I(A, B);
					};
				} else {
					var s = w.beforeCreate;
					w.beforeCreate = s ? [].concat(s, n) : [n];
				}
			return { exports: A, options: w };
		}
		E.d(B, "a", function () {
			return g;
		});
	},
	1: function (A, B) {
		A.exports = { "@VERSION": 2 };
	},
	10: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var g,
			Q = (g = E(17)) && g.__esModule ? g : { default: g };
		weex
			.requireModule("dom")
			.addRule("fontFace", {
				fontFamily: "uniicons",
				src: "url('data:font/truetype;charset=utf-8;base64,')",
			});
		var t = {
			name: "UniIcons",
			props: {
				type: { type: String, default: "" },
				color: { type: String, default: "#333333" },
				size: { type: [Number, String], default: 16 },
				customIcons: { type: String, default: "" },
			},
			data: function () {
				return { icons: Q.default };
			},
			methods: {
				_onClick: function () {
					this.$emit("click");
				},
			},
		};
		B.default = t;
	},
	11: function (A, B) {
		A.exports = {
			".uni-icons": { "": { fontFamily: ["uniicons", 0, 0, 0], textDecoration: ["none", 0, 0, 0], textAlign: ["center", 0, 0, 0] } },
			"@VERSION": 2,
		};
	},
	12: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return g;
		}),
			E.d(B, "c", function () {
				return Q;
			}),
			E.d(B, "a", function () {});
		var g = function () {
				var A = this,
					B = A.$createElement;
				return (A._self._c || B)(
					"u-text",
					{
						staticClass: ["uni-icons"],
						class: [A.customIcons, A.customIcons ? A.type : ""],
						style: { color: A.color, "font-size": A.size + "px" },
						appendAsTree: !0,
						attrs: { append: "tree" },
						on: { click: A._onClick },
					},
					[A._v(A._s(A.icons[A.type]))],
				);
			},
			Q = [];
	},
	13: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(5),
			Q = E(2);
		for (var t in Q)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return Q[A];
					});
				})(t);
		var c = E(0);
		var e = Object(c.a)(Q.default, g.b, g.c, !1, null, "168bca6c", "39eb9647", !1, g.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(8).default, this.options.style) : Object.assign(this.options.style, E(8).default);
		}).call(e),
			(B.default = e.exports);
	},
	14: function (A, B, E) {
		"use strict";
		function g(A) {
			var B = Object.prototype.toString.call(A);
			return B.substring(8, B.length - 1);
		}
		function Q() {
			return "string" == typeof __channelId__ && __channelId__;
		}
		function t(A, B) {
			switch (g(B)) {
				case "Function":
					return "function() { [native code] }";
				default:
					return B;
			}
		}
		Object.defineProperty(B, "__esModule", { value: !0 }),
			(B.log = function (A) {
				for (var B = arguments.length, E = new Array(B > 1 ? B - 1 : 0), g = 1; g < B; g++) E[g - 1] = arguments[g];
				console[A].apply(console, E);
			}),
			(B.default = function () {
				for (var A = arguments.length, B = new Array(A), E = 0; E < A; E++) B[E] = arguments[E];
				var c = B.shift();
				if (Q()) return B.push(B.pop().replace("at ", "uni-app:///")), console[c].apply(console, B);
				var e = B.map(function (A) {
						var B = Object.prototype.toString.call(A).toLowerCase();
						if ("[object object]" === B || "[object array]" === B)
							try {
								A = "---BEGIN:JSON---" + JSON.stringify(A, t) + "---END:JSON---";
							} catch (E) {
								A = B;
							}
						else if (null === A) A = "---NULL---";
						else if (void 0 === A) A = "---UNDEFINED---";
						else {
							var E = g(A).toUpperCase();
							A = "NUMBER" === E || "BOOLEAN" === E ? "---BEGIN:" + E + "---" + A + "---END:" + E + "---" : String(A);
						}
						return A;
					}),
					o = "";
				if (e.length > 1) {
					var i = e.pop();
					(o = e.join("---COMMA---")), 0 === i.indexOf(" at ") ? (o += i) : (o += "---COMMA---" + i);
				} else o = e[0];
				console[c](o);
			});
	},
	15: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(11),
			Q = E.n(g);
		for (var t in g)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return g[A];
					});
				})(t);
		B.default = Q.a;
	},
	16: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(12),
			Q = E(9);
		for (var t in Q)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return Q[A];
					});
				})(t);
		var c = E(0);
		var e = Object(c.a)(Q.default, g.b, g.c, !1, null, "041e7dc0", "67d6d51e", !1, g.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(15).default, this.options.style) : Object.assign(this.options.style, E(15).default);
		}).call(e),
			(B.default = e.exports);
	},
	17: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		B.default = {
			pulldown: "\ue588",
			refreshempty: "\ue461",
			back: "\ue471",
			forward: "\ue470",
			more: "\ue507",
			"more-filled": "\ue537",
			scan: "\ue612",
			qq: "\ue264",
			weibo: "\ue260",
			weixin: "\ue261",
			pengyouquan: "\ue262",
			loop: "\ue565",
			refresh: "\ue407",
			"refresh-filled": "\ue437",
			arrowthindown: "\ue585",
			arrowthinleft: "\ue586",
			arrowthinright: "\ue587",
			arrowthinup: "\ue584",
			"undo-filled": "\ue7d6",
			undo: "\ue406",
			redo: "\ue405",
			"redo-filled": "\ue7d9",
			bars: "\ue563",
			chatboxes: "\ue203",
			camera: "\ue301",
			"chatboxes-filled": "\ue233",
			"camera-filled": "\ue7ef",
			"cart-filled": "\ue7f4",
			cart: "\ue7f5",
			"checkbox-filled": "\ue442",
			checkbox: "\ue7fa",
			arrowleft: "\ue582",
			arrowdown: "\ue581",
			arrowright: "\ue583",
			"smallcircle-filled": "\ue801",
			arrowup: "\ue580",
			circle: "\ue411",
			"eye-filled": "\ue568",
			"eye-slash-filled": "\ue822",
			"eye-slash": "\ue823",
			eye: "\ue824",
			"flag-filled": "\ue825",
			flag: "\ue508",
			"gear-filled": "\ue532",
			reload: "\ue462",
			gear: "\ue502",
			"hand-thumbsdown-filled": "\ue83b",
			"hand-thumbsdown": "\ue83c",
			"hand-thumbsup-filled": "\ue83d",
			"heart-filled": "\ue83e",
			"hand-thumbsup": "\ue83f",
			heart: "\ue840",
			home: "\ue500",
			info: "\ue504",
			"home-filled": "\ue530",
			"info-filled": "\ue534",
			"circle-filled": "\ue441",
			"chat-filled": "\ue847",
			chat: "\ue263",
			"mail-open-filled": "\ue84d",
			"email-filled": "\ue231",
			"mail-open": "\ue84e",
			email: "\ue201",
			checkmarkempty: "\ue472",
			list: "\ue562",
			"locked-filled": "\ue856",
			locked: "\ue506",
			"map-filled": "\ue85c",
			"map-pin": "\ue85e",
			"map-pin-ellipse": "\ue864",
			map: "\ue364",
			"minus-filled": "\ue440",
			"mic-filled": "\ue332",
			minus: "\ue410",
			micoff: "\ue360",
			mic: "\ue302",
			clear: "\ue434",
			smallcircle: "\ue868",
			close: "\ue404",
			closeempty: "\ue460",
			paperclip: "\ue567",
			paperplane: "\ue503",
			"paperplane-filled": "\ue86e",
			"person-filled": "\ue131",
			"contact-filled": "\ue130",
			person: "\ue101",
			contact: "\ue100",
			"images-filled": "\ue87a",
			phone: "\ue200",
			images: "\ue87b",
			image: "\ue363",
			"image-filled": "\ue877",
			"location-filled": "\ue333",
			location: "\ue303",
			"plus-filled": "\ue439",
			plus: "\ue409",
			plusempty: "\ue468",
			"help-filled": "\ue535",
			help: "\ue505",
			"navigate-filled": "\ue884",
			navigate: "\ue501",
			"mic-slash-filled": "\ue892",
			search: "\ue466",
			settings: "\ue560",
			sound: "\ue590",
			"sound-filled": "\ue8a1",
			"spinner-cycle": "\ue465",
			"download-filled": "\ue8a4",
			"personadd-filled": "\ue132",
			"videocam-filled": "\ue8af",
			personadd: "\ue102",
			upload: "\ue402",
			"upload-filled": "\ue8b1",
			starhalf: "\ue463",
			"star-filled": "\ue438",
			star: "\ue408",
			trash: "\ue401",
			"phone-filled": "\ue230",
			compose: "\ue400",
			videocam: "\ue300",
			"trash-filled": "\ue8dc",
			download: "\ue403",
			"chatbubble-filled": "\ue232",
			chatbubble: "\ue202",
			"cloud-download": "\ue8e4",
			"cloud-upload-filled": "\ue8e5",
			"cloud-upload": "\ue8e6",
			"cloud-download-filled": "\ue8e9",
			headphones: "\ue8bf",
			shop: "\ue609",
		};
	},
	2: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(3),
			Q = E.n(g);
		for (var t in g)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return g[A];
					});
				})(t);
		B.default = Q.a;
	},
	231: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(232),
			Q = E.n(g);
		for (var t in g)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return g[A];
					});
				})(t);
		B.default = Q.a;
	},
	232: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var g = {
			name: "UniGoodsNav",
			props: {
				options: {
					type: Array,
					default: function () {
						return [
							{ icon: "shop", text: "\u5e97\u94fa" },
							{ icon: "cart", text: "\u8d2d\u7269\u8f66" },
						];
					},
				},
				buttonGroup: {
					type: Array,
					default: function () {
						return [
							{ text: "\u52a0\u5165\u8d2d\u7269\u8f66", backgroundColor: "#ffa200", color: "#fff" },
							{ text: "\u7acb\u5373\u8d2d\u4e70", backgroundColor: "#ff0000", color: "#fff" },
						];
					},
				},
				fill: { type: Boolean, default: !1 },
			},
			methods: {
				onClick: function (A, B) {
					this.$emit("click", { index: A, content: B });
				},
				buttonClick: function (A, B) {
					uni.report && uni.report(B.text, B.text), this.$emit("buttonClick", { index: A, content: B });
				},
			},
		};
		B.default = g;
	},
	233: function (A, B) {
		A.exports = {
			".flex": { "": { flexDirection: ["row", 0, 0, 0] } },
			".uni-goods-nav": { "": { flex: [1, 0, 0, 1], flexDirection: ["row", 0, 0, 1] } },
			".uni-tab__cart-box": { "": { flex: [1, 0, 0, 2], height: ["50", 0, 0, 2], backgroundColor: ["#ffffff", 0, 0, 2], zIndex: [900, 0, 0, 2] } },
			".uni-tab__cart-sub-left": { "": { paddingTop: [0, 0, 0, 3], paddingRight: ["5", 0, 0, 3], paddingBottom: [0, 0, 0, 3], paddingLeft: ["5", 0, 0, 3] } },
			".uni-tab__cart-sub-right": { "": { flex: [1, 0, 0, 4] } },
			".uni-tab__right": {
				"": {
					marginTop: ["5", 0, 0, 5],
					marginRight: ["10", 0, 0, 5],
					marginBottom: ["5", 0, 0, 5],
					marginLeft: [0, 0, 0, 5],
					borderRadius: ["100", 0, 0, 5],
					overflow: ["hidden", 0, 0, 5],
				},
			},
			".uni-tab__cart-button-left": {
				"": {
					position: ["relative", 0, 0, 6],
					justifyContent: ["center", 0, 0, 6],
					alignItems: ["center", 0, 0, 6],
					flexDirection: ["column", 0, 0, 6],
					marginTop: [0, 0, 0, 6],
					marginRight: ["10", 0, 0, 6],
					marginBottom: [0, 0, 0, 6],
					marginLeft: ["10", 0, 0, 6],
				},
			},
			".uni-tab__icon": { "": { width: ["18", 0, 0, 7], height: ["18", 0, 0, 7] } },
			".image": { "": { width: ["18", 0, 0, 8], height: ["18", 0, 0, 8] } },
			".uni-tab__text": { "": { marginTop: ["3", 0, 0, 9], fontSize: ["12", 0, 0, 9], color: ["#646566", 0, 0, 9] } },
			".uni-tab__cart-button-right": {
				"": { flex: [1, 0, 0, 10], justifyContent: ["center", 0, 0, 10], alignItems: ["center", 0, 0, 10], "opacity:active": [0.7, 0, 0, 12] },
			},
			".uni-tab__cart-button-right-text": { "": { fontSize: ["14", 0, 0, 11], color: ["#ffffff", 0, 0, 11] } },
			".uni-tab__dot-box": {
				"": {
					position: ["absolute", 0, 0, 13],
					right: ["-2", 0, 0, 13],
					top: ["2", 0, 0, 13],
					justifyContent: ["center", 0, 0, 13],
					alignItems: ["center", 0, 0, 13],
				},
			},
			".uni-tab__dot": {
				"": {
					paddingTop: [0, 0, 0, 14],
					paddingRight: ["4", 0, 0, 14],
					paddingBottom: [0, 0, 0, 14],
					paddingLeft: ["4", 0, 0, 14],
					lineHeight: ["15", 0, 0, 14],
					color: ["#ffffff", 0, 0, 14],
					textAlign: ["center", 0, 0, 14],
					fontSize: ["12", 0, 0, 14],
					backgroundColor: ["#ff0000", 0, 0, 14],
					borderRadius: ["15", 0, 0, 14],
				},
			},
			".uni-tab__dots": {
				"": {
					paddingTop: [0, 0, 0, 15],
					paddingRight: ["4", 0, 0, 15],
					paddingBottom: [0, 0, 0, 15],
					paddingLeft: ["4", 0, 0, 15],
					borderRadius: ["15", 0, 0, 15],
				},
			},
			".uni-tab__color-y": { "": { backgroundColor: ["#ffa200", 0, 0, 16] } },
			".uni-tab__color-r": { "": { backgroundColor: ["#ff0000", 0, 0, 17] } },
			"@VERSION": 2,
		};
	},
	234: function (A, B, E) {
		"use strict";
		var g = E(235),
			Q = E.n(g);
		B.default = Q.a;
	},
	235: function (A, B, E) {
		"use strict";
		(function (A) {
			Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
			var E = {
				components: {},
				data: function () {
					return {
						options: [
							{ icon: "headphones", text: "\u5ba2\u670d" },
							{ icon: "shop", text: "\u5e97\u94fa", info: 2, infoBackgroundColor: "#007aff", infoColor: "#f5f5f5" },
							{ icon: "cart", text: "\u8d2d\u7269\u8f66", info: 2 },
						],
						buttonGroup: [
							{ text: "\u52a0\u5165\u8d2d\u7269\u8f66", backgroundColor: "#ffa200", color: "#fff" },
							{ text: "\u7acb\u5373\u8d2d\u4e70", backgroundColor: "#ff0000", color: "#fff" },
						],
					};
				},
				onLoad: function () {},
				methods: {
					onClick: function (A) {
						uni.showToast({ title: "\u70b9\u51fb".concat(A.content.text), icon: "none" });
					},
					buttonClick: function (B) {
						A("log", B, " at pages/extUI/goods-nav/goods-nav.nvue:61"), this.options[2].info++;
					},
				},
			};
			B.default = E;
		}).call(this, E(14).default);
	},
	236: function (A, B) {
		A.exports = {
			".example": { "": { paddingTop: [0, 0, 0, 2], paddingRight: ["15", 0, 0, 2], paddingBottom: [0, 0, 0, 2], paddingLeft: ["15", 0, 0, 2] } },
			".example-info": {
				"": {
					paddingTop: ["15", 0, 0, 3],
					paddingRight: ["15", 0, 0, 3],
					paddingBottom: ["15", 0, 0, 3],
					paddingLeft: ["15", 0, 0, 3],
					color: ["#3b4144", 0, 0, 3],
					backgroundColor: ["#ffffff", 0, 0, 3],
					fontSize: ["14", 0, 0, 3],
					lineHeight: ["20", 0, 0, 3],
				},
			},
			".example-info-text": { "": { fontSize: ["14", 0, 0, 4], lineHeight: ["20", 0, 0, 4], color: ["#3b4144", 0, 0, 4] } },
			".example-body": {
				"": {
					flexDirection: ["column", 0, 0, 5],
					paddingTop: [0, 0, 0, 9],
					paddingRight: [0, 0, 0, 9],
					paddingBottom: [0, 0, 0, 9],
					paddingLeft: [0, 0, 0, 9],
					backgroundColor: ["#ffffff", 0, 0, 5],
				},
			},
			".word-btn-white": { "": { fontSize: ["18", 0, 0, 6], color: ["#FFFFFF", 0, 0, 6] } },
			".word-btn": {
				"": {
					flexDirection: ["row", 0, 0, 7],
					alignItems: ["center", 0, 0, 7],
					justifyContent: ["center", 0, 0, 7],
					borderRadius: ["6", 0, 0, 7],
					height: ["48", 0, 0, 7],
					marginTop: ["15", 0, 0, 7],
					marginRight: ["15", 0, 0, 7],
					marginBottom: ["15", 0, 0, 7],
					marginLeft: ["15", 0, 0, 7],
					backgroundColor: ["#007AFF", 0, 0, 7],
				},
			},
			".word-btn--hover": { "": { backgroundColor: ["#4ca2ff", 0, 0, 8] } },
			".goods-carts": {
				"": { flexDirection: ["column", 0, 0, 10], position: ["fixed", 0, 0, 10], left: [0, 0, 0, 10], right: [0, 0, 0, 10], bottom: [0, 0, 0, 10] },
			},
			"@VERSION": 2,
		};
	},
	3: function (A, B, E) {
		"use strict";
		Object.defineProperty(B, "__esModule", { value: !0 }), (B.default = void 0);
		var g = {
			name: "UniSection",
			props: { type: { type: String, default: "" }, title: { type: String, default: "" }, subTitle: { type: String, default: "" } },
			data: function () {
				return {};
			},
			watch: {
				title: function (A) {
					uni.report && "" !== A && uni.report("title", A);
				},
			},
			methods: {
				onClick: function () {
					this.$emit("click");
				},
			},
		};
		B.default = g;
	},
	4: function (A, B) {
		A.exports = {
			".uni-section": {
				"": {
					position: ["relative", 0, 0, 0],
					marginTop: ["10", 0, 0, 0],
					flexDirection: ["row", 0, 0, 0],
					alignItems: ["center", 0, 0, 0],
					paddingTop: [0, 0, 0, 0],
					paddingRight: ["10", 0, 0, 0],
					paddingBottom: [0, 0, 0, 0],
					paddingLeft: ["10", 0, 0, 0],
					height: ["50", 0, 0, 0],
					backgroundColor: ["#f8f8f8", 0, 0, 0],
					fontWeight: ["normal", 0, 0, 0],
				},
			},
			".uni-section__head": {
				"": { flexDirection: ["row", 0, 0, 1], justifyContent: ["center", 0, 0, 1], alignItems: ["center", 0, 0, 1], marginRight: ["10", 0, 0, 1] },
			},
			".line": { "": { height: ["15", 0, 0, 2], backgroundColor: ["#c0c0c0", 0, 0, 2], borderRadius: ["5", 0, 0, 2], width: ["3", 0, 0, 2] } },
			".circle": {
				"": {
					width: ["8", 0, 0, 3],
					height: ["8", 0, 0, 3],
					borderTopRightRadius: ["50", 0, 0, 3],
					borderTopLeftRadius: ["50", 0, 0, 3],
					borderBottomLeftRadius: ["50", 0, 0, 3],
					borderBottomRightRadius: ["50", 0, 0, 3],
					backgroundColor: ["#c0c0c0", 0, 0, 3],
				},
			},
			".uni-section__content": { "": { flexDirection: ["column", 0, 0, 4], flex: [1, 0, 0, 4], color: ["#333333", 0, 0, 4] } },
			".uni-section__content-title": { "": { fontSize: ["14", 0, 0, 5], color: ["#333333", 0, 0, 5] } },
			".distraction": { "": { flexDirection: ["row", 0, 0, 6], alignItems: ["center", 0, 0, 6] } },
			".uni-section__content-sub": { "": { fontSize: ["12", 0, 0, 7], color: ["#999999", 0, 0, 7] } },
			"@VERSION": 2,
		};
	},
	453: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return Q;
		}),
			E.d(B, "c", function () {
				return t;
			}),
			E.d(B, "a", function () {
				return g;
			});
		var g = { uniSection: E(13).default, uniGoodsNav: E(679).default },
			Q = function () {
				var A = this,
					B = A.$createElement,
					E = A._self._c || B;
				return E("scroll-view", { staticStyle: { flexDirection: "column" }, attrs: { scrollY: !0, showScrollbar: !0, enableBackToTop: !0, bubble: "true" } }, [
					E(
						"view",
						[
							E("u-text", { staticClass: ["example-info"], appendAsTree: !0, attrs: { append: "tree" } }, [
								A._v(
									" uni-goods-nav \u7ec4\u4ef6\u4e3b\u8981\u7528\u4e8e\u7535\u5546\u7c7b\u5e94\u7528\u5e95\u90e8\u5bfc\u822a\uff0c\u53ef\u81ea\u5b9a\u4e49\u52a0\u5165\u8d2d\u7269\u8f66\uff0c\u8d2d\u4e70\u7b49\u64cd\u4f5c",
								),
							]),
							E("uni-section", { attrs: { title: "\u57fa\u7840\u7528\u6cd5", type: "line" } }),
							E("view", { staticClass: ["example-body"] }, [E("uni-goods-nav", { on: { click: A.onClick } })], 1),
							E("uni-section", { attrs: { title: "\u81ea\u5b9a\u4e49\u7528\u6cd5", type: "line" } }),
							E(
								"view",
								{ staticClass: ["example-body"] },
								[
									E("uni-goods-nav", {
										attrs: { fill: !0, options: A.options, buttonGroup: A.buttonGroup },
										on: { click: A.onClick, buttonClick: A.buttonClick },
									}),
								],
								1,
							),
							E(
								"view",
								{ staticClass: ["goods-carts"] },
								[
									E("uni-goods-nav", {
										attrs: { options: A.options, fill: !0, buttonGroup: A.buttonGroup },
										on: { click: A.onClick, buttonClick: A.buttonClick },
									}),
								],
								1,
							),
						],
						1,
					),
				]);
			},
			t = [];
	},
	5: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return g;
		}),
			E.d(B, "c", function () {
				return Q;
			}),
			E.d(B, "a", function () {});
		var g = function () {
				var A = this,
					B = A.$createElement,
					E = A._self._c || B;
				return E(
					"view",
					{ staticClass: ["uni-section"], attrs: { nvue: !0 } },
					[
						A.type ? E("view", { staticClass: ["uni-section__head"] }, [E("view", { staticClass: ["uni-section__head-tag"], class: A.type })]) : A._e(),
						E("view", { staticClass: ["uni-section__content"] }, [
							E("u-text", { staticClass: ["uni-section__content-title"], class: { distraction: !A.subTitle }, appendAsTree: !0, attrs: { append: "tree" } }, [
								A._v(A._s(A.title)),
							]),
							A.subTitle
								? E("u-text", { staticClass: ["uni-section__content-sub"], appendAsTree: !0, attrs: { append: "tree" } }, [A._v(A._s(A.subTitle))])
								: A._e(),
						]),
						A._t("default"),
					],
					2,
				);
			},
			Q = [];
	},
	505: function (A, B, E) {
		"use strict";
		E.d(B, "b", function () {
			return Q;
		}),
			E.d(B, "c", function () {
				return t;
			}),
			E.d(B, "a", function () {
				return g;
			});
		var g = { uniIcons: E(16).default },
			Q = function () {
				var A = this,
					B = A.$createElement,
					E = A._self._c || B;
				return E("view", { staticClass: ["uni-goods-nav"] }, [
					E("view", { staticClass: ["uni-tab__seat"] }),
					E("view", { staticClass: ["uni-tab__cart-box", "flex"] }, [
						E(
							"view",
							{ staticClass: ["flex", "uni-tab__cart-sub-left"] },
							A._l(A.options, function (B, g) {
								return E(
									"view",
									{
										key: g,
										staticClass: ["flex", "uni-tab__cart-button-left", "uni-tab__shop-cart"],
										on: {
											click: function (E) {
												A.onClick(g, B);
											},
										},
									},
									[
										E("view", { staticClass: ["uni-tab__icon"] }, [E("uni-icons", { attrs: { type: B.icon, size: "20", color: "#646566" } })], 1),
										E("u-text", { staticClass: ["uni-tab__text"], appendAsTree: !0, attrs: { append: "tree" } }, [A._v(A._s(B.text))]),
										E("view", { staticClass: ["flex", "uni-tab__dot-box"] }, [
											B.info
												? E(
														"u-text",
														{
															staticClass: ["uni-tab__dot"],
															class: { "uni-tab__dots": B.info > 9 },
															style: { backgroundColor: B.infoBackgroundColor ? B.infoBackgroundColor : "#ff0000", color: B.infoColor ? B.infoColor : "#fff" },
															appendAsTree: !0,
															attrs: { append: "tree" },
														},
														[A._v(A._s(B.info))],
												  )
												: A._e(),
										]),
									],
								);
							}),
							0,
						),
						E(
							"view",
							{ staticClass: ["flex", "uni-tab__cart-sub-right"], class: { "uni-tab__right": A.fill } },
							A._l(A.buttonGroup, function (B, g) {
								return E(
									"view",
									{
										key: g,
										staticClass: ["flex", "uni-tab__cart-button-right"],
										style: { backgroundColor: B.backgroundColor, color: B.color },
										on: {
											click: function (E) {
												A.buttonClick(g, B);
											},
										},
									},
									[
										E("u-text", { staticClass: ["uni-tab__cart-button-right-text"], style: { color: B.color }, appendAsTree: !0, attrs: { append: "tree" } }, [
											A._v(A._s(B.text)),
										]),
									],
								);
							}),
							0,
						),
					]),
				]);
			},
			t = [];
	},
	576: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(233),
			Q = E.n(g);
		for (var t in g)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return g[A];
					});
				})(t);
		B.default = Q.a;
	},
	577: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(236),
			Q = E.n(g);
		for (var t in g)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return g[A];
					});
				})(t);
		B.default = Q.a;
	},
	6: function (A, B, E) {
		(Vue.prototype.__$appStyle__ = {}), Vue.prototype.__merge_style && Vue.prototype.__merge_style(E(7).default, Vue.prototype.__$appStyle__);
	},
	678: function (A, B, E) {
		"use strict";
		E.r(B);
		E(6);
		var g = E(91);
		"undefined" == typeof Promise ||
			Promise.prototype.finally ||
			(Promise.prototype.finally = function (A) {
				var B = this.constructor;
				return this.then(
					function (E) {
						return B.resolve(A()).then(function () {
							return E;
						});
					},
					function (E) {
						return B.resolve(A()).then(function () {
							throw E;
						});
					},
				);
			}),
			(g.default.mpType = "page"),
			(g.default.route = "pages/extUI/goods-nav/goods-nav"),
			(g.default.el = "#root"),
			new Vue(g.default);
	},
	679: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(505),
			Q = E(231);
		for (var t in Q)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return Q[A];
					});
				})(t);
		var c = E(0);
		var e = Object(c.a)(Q.default, g.b, g.c, !1, null, "49abb10d", "ec5418d6", !1, g.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(576).default, this.options.style) : Object.assign(this.options.style, E(576).default);
		}).call(e),
			(B.default = e.exports);
	},
	7: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(1),
			Q = E.n(g);
		for (var t in g)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return g[A];
					});
				})(t);
		B.default = Q.a;
	},
	8: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(4),
			Q = E.n(g);
		for (var t in g)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return g[A];
					});
				})(t);
		B.default = Q.a;
	},
	9: function (A, B, E) {
		"use strict";
		E.r(B);
		var g = E(10),
			Q = E.n(g);
		for (var t in g)
			"default" !== t &&
				(function (A) {
					E.d(B, A, function () {
						return g[A];
					});
				})(t);
		B.default = Q.a;
	},
	91: function (A, B, E) {
		"use strict";
		var g = E(453),
			Q = E(234),
			t = E(0);
		var c = Object(t.a)(Q.default, g.b, g.c, !1, null, null, "0e9ec0fa", !1, g.a, void 0);
		(function (A) {
			this.options.style || (this.options.style = {}),
				Vue.prototype.__merge_style && Vue.prototype.__$appStyle__ && Vue.prototype.__merge_style(Vue.prototype.__$appStyle__, this.options.style),
				Vue.prototype.__merge_style ? Vue.prototype.__merge_style(E(577).default, this.options.style) : Object.assign(this.options.style, E(577).default);
		}).call(c),
			(B.default = c.exports);
	},
});
