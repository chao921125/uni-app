<template>
	<view class="s-page-wrapper" v-if="showPage" style="background-color: #ebebeb;">
		<view class="index-goods" v-if="loadGoods">
			...
			<view class="goods_info" style="padding-top: 6px;padding-bottom: 16upx">
				<view class="coupon-price s-row">
					<view class="price" style="width: 70%">
						...
						<text
							style="font-size: 24upx;color: grey;text-decoration:line-through;margin-left: 8px">￥{{ goods.itemprice }}</text>
					</view>
				...
				<view style="width: 100%;padding:30upx">
					...
					<view class="volume" style="font-size: 28upx;float: right;color:#333333;"><text
							style="color:#E20C0A;font-weight: bold;">{{ goods.itemsale }}人付款</text></view>
			...
			<view class="goods_quan s-row" style="padding-top: 8upx;padding-bottom: 8upx;background-color: #F2F2F2;">
				<view class="row getGoodsLink" @tap="shopCartShare('quan')">
					<view class="is-col-16 money" style="color: #DE2E2D;">
						...
						<view class="date-coupon" style="color: #DE2E2D;" v-if="goods.start_time">使用期限:
							{{ goods.start_time }} - {{ goods.end_time }}
						</view>
					</view>
			...
			<view></view>
			<view style="display: flex;background-color: #fff;padding: 16upx;">
				<view v-if="couponlist.length > 0" style="color:#333333;width: 86%; font-size: 32upx;">
					为你推荐
				</view>
			</view>

			<view class="goods-list" style="background-color: #fff;height: 670upx;width: 100%;"
				v-if="couponlist.length > 0">
				<orange-goods-card-home-two v-for="(item, index) in couponlist" :tkmoney="item.tkmoney"
					:tkmoneys="item.tkmoney" :itemid="item.itemid" :shopname='item.shopname'
					...
					:couponmoney="item.couponmoney"></orange-goods-card-home-two>
			</view>
			<!-- 购买按钮 -->
			<view class="goods_shop_cart">
				<view class="cent" style="display: flex">
					...
					<view style="width: 75%;display: flex;color: white">
						<view
							style="border-radius: 32upx;width: 50%;background: #e43939;text-align: center;padding-top:8px;margin: 4px"
							@tap="shopCartShares('tkl')">{{ buyDes }}</view>
						<view class="getTbk"
							style="border-radius: 32upx;width: 50%;background:#E41F19; text-align: center;padding-top: 8px;margin: 4px"
			...
			<view class="navBarButtonBox">
				...
				<simpleModal ref="simpleModalTkl" @maskClose="TklmaskClose">
					...
					<view class="buy-box-center">
						...
						<view class="buy-btn-copy" v-bind:class="[copyTklStatus ? 'active' : '', '']"
			...
			<!-- 宝贝详情 -->
			<view class="goods_reco"
				style="text-align: center;font-size: 36upx;color: #333333;background: white;margin-top:6upx;padding: 20upx;"
				v-if="goods.images">
				<view class="imglist">
			...
			<view class="content">
				<view class="flex_row_c_c modalView" :class="qrShow ? 'show' : ''" @tap="hideQr()">
					<view class="flex_column">
						...
						<button class="marginTop2vh" type="primary" size="mini"
							@tap.prevent.stop="saveImage()">保存图片</button>
					</view>
				</view>
			...
			<view class="cu-modal" :class="modalName == 'Image' ? 'show' : ''" @tap="hideModal">
				<view class="cu-dialog" v-if="goods && erweimapath && haibaoShow" @tap="hideModal">
					<view class="bg-img">
						<wm-poster @success="posterSuccess" :imgSrc="goods.itempic" :QrSrc="erweimapath"
			...
			loadGoodsInfo: function() {
				...
				this.$Request.get('/api/item_detail/apikey/maxd/itemid/' + this.itemid).then(res => {
					if (res.code === 1) {
						...
						res.data.itemsale = res.data.itemsale > 10000 ? (res.data.itemsale / 10000).toFixed(
							1) + '万' : res.data.itemsale;
						...
						that.shengji = parseFloat(res.data.tkmoney * parseFloat(this.$queue.maxMoney()))
							.toFixed(2);
						that.grade = grade;
						try {
							...
							if (this.isEnable != '否') {
								if (grade) {
									if (grade === this.maxMoney) {
										this.buyDes = '返现 ￥' + (this.goods.tkmoney * parseFloat(grade))
											.toFixed(2);
									} else {
										this.buyDes = '升级返 ￥' + (this.goods.tkmoney * parseFloat(this.$queue
											.maxMoney())).toFixed(2);
									}
								} else {
									this.buyDes = '返现 ￥' + (this.goods.tkmoney * parseFloat(this.$queue
										.minMoney())).toFixed(2);
								}
							} else {
								this.buyDes = '分享给好友';
							...
							that.$Request.get('/tao/wx/js/sign?url=' + url).then(res => {
								...
								that.$wx.ready(function() {
									...
									that.$wx.onMenuShareAppMessage({
										title: that.goods.itemendprice + '元 ' + that
											.goods.itemtitle,
				...
				this.$Request.get('/api/get_similar_info/apikey/maxd/itemid/' + this.itemid).then(res => {
					if (res.code === 1) {
						for (let i = 0; i < 6; i++) {
							res.data[i].itemsale = res.data[i].itemsale >= 10000 ? '已售 ' + (res.data[i]
									.itemsale / 10000).toFixed(1) +
								'万' : '已售 ' + res.data[i].itemsale;
					...
					.then(res => {
						if (res.code === 1) {
							...
							if (relation_id || relation) {
								...
								coupon_click_url = res.data.coupon_click_url + '&relationId=' + this.$queue
									.getInvitation();
							}
							//#ifdef APP-PLUS
						...
						.then(res => {
							if (res.code === 1) {
								...
								if (relation_id || relation) {
									...
									if (relation_id) {
										coupon_click_url = res.data.coupon_click_url + '&relationId=' +
											relation_id;
									}
								} else {
									coupon_click_url = res.data.coupon_click_url + '&relationId=' + this.$queue
							...
							this.$Request
								...
								.then(res => {
									if (res && res.tbk_tpwd_create_response.data.model) {
										...
										// this.tkl = res.tbk_tpwd_create_response.data.model;
										this.erweima = '复制这段内容(' + res.tbk_tpwd_create_response.data
											.password_simple + ')\n打开【手淘】领券';
									}
								});
						}
					...
					if (!this.poster.finalPath) {
						const d = await getSharePoster({
							...
							qrCodeArray: ({
								...
								if (relation_id) {
									return [{
										text: this.$queue.publicYuMing() +
											'/pages/detail/goodsinfo?itemid=' + this.itemid +
											'&relation_id=' +
											relation_id,
							...
							textArray: ({
								...
								return [{
										...
										infoCallBack(textLength) {
											_app.log('index页面的text的infocallback ，textlength:' +
												textLength);
											return {
							...
							setDraw: ({
								...
								Context.fillRect(0, bgObj.height - bgObj.height * 0.2, bgObj.width, bgObj
									.height * 0.2);
							}
						});
