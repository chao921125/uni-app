  ...
  <view>
    ...
    <view class="person-box">
      <image
        ...
        mode="aspectFit"
      />
      <open-data class="avatar image-round" type="userAvatarUrl" v-if="avatarShow"></open-data>
      <image
        class="group-joiner"
    ...
    <view class="invita-btn" @tap="goGroupOrder">立即参团</view>
  </view>
</template>
<script>
...
export default {
  ...
  onLoad(option) {
    if (JSON.stringify(option) != '{}') {
      uni.setStorageSync('invitaObj', option);
    }
  },
  methods: {
    pageInit() { },
    ...
    goGroupOrder() {
      this.$uni.navigateTo({
        url: `/pages2/order/group-order?isJoiner=joiner`
      });
    }
  }
};
