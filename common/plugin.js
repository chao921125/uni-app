...
export function isOpenSetting(type) {
	...
	return new Promise((resolve, reject) => {
		...
		switch (type) {
			case 1:
				key = "scope.userInfo";
				break; // 用户信息 uni.getUserInfo
			case 2:
				key = "scope.userLocation";
				break; // 地理位置 uni.getLocation, uni.chooseLocation
			case 3:
				key = "scope.userLocationBackground";
				break; // 后台定位 wx.userLocationBackground
			case 4:
				key = "scope.address";
				break; // 通信地址 uni.chooseAddress
			case 5:
				key = "scope.record";
				break; // 录音功能 uni.getRecorderManager
			case 6:
				key = "scope.writePhotosAlbum";
				break; // 保存到相册 uni.saveImageToPhotosAlbum, uni.saveVideoToPhotosAlbum
			case 7:
				key = "scope.camera";
				break; // 摄像头 <camera />
			case 8:
				key = "scope.invoice";
				break; // 获取发票 wx.chooseInvoice
			case 9:
				key = "scope.invoiceTitle";
				break; // 发票抬头 uni.chooseInvoiceTitle
			case 10:
				key = "scope.werun";
				break; // 微信运动步数 wx.getWeRunData
			default:
				key = "";
				break;
		}
